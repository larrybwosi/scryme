// src-tauri/src/api_config.rs

pub mod routes {
    // --- Auth ---
    pub const CHECK_IN: &str = "api/v3/:orgSlug/pos/login";
    pub const CHECK_OUT: &str = "api/v3/:orgSlug/pos/check-out";
    pub const LOCATIONS: &str = "api/v3/:orgSlug/pos/locations";
    pub const ABLY_AUTH: &str = "api/v3/:orgSlug/pos/ably-auth";
    pub const MPESA_INITIATE: &str = "api/v3/:orgSlug/payments/stkpush";

    // --- Inventory / Stock ---
    pub const INVENTORY_TRANSFERS: &str = "api/v3/:orgSlug/stocking/transfers";
    pub const INVENTORY_REQUESTS: &str = "api/v3/:orgSlug/stocking/requests";
    pub const INVENTORY_PROCESS: &str = "api/v3/:orgSlug/stocking/transfers";
    pub const INCOMING_SHIPMENTS: &str = "api/v3/:orgSlug/stocking/purchases";

    pub fn purchase_receive(id: &str) -> String {
        format!("api/v3/:orgSlug/stocking/purchases/{}/receive", id)
    }

    pub fn transfer_receive(id: &str) -> String {
        format!("api/v3/:orgSlug/stocking/transfers/{}/receive", id)
    }

    // --- Sales ---
    pub const SALE_PROCESS: &str = "api/v3/:orgSlug/pos/sale";
    pub const SALE_BASE: &str = "api/v3/:orgSlug/pos/sale";
    pub const SALE_PAYMENTS: &str = "api/v3/:orgSlug/pos/sale/payments";
    pub const TRANSACTION_SCAN: &str = "api/v3/:orgSlug/pos/transactions";
    pub const ORDERS: &str = "api/v3/:orgSlug/pos/orders";

    // --- Products & Pricing ---
    pub const PRODUCTS: &str = "api/v3/:orgSlug/pos/products";
    pub const PRICING: &str = "api/v3/:orgSlug/pos/pricing";
    pub const PRICING_SYNC: &str = "api/v3/:orgSlug/pos/pricing/sync";

    // --- Customers ---
    pub const CUSTOMERS: &str = "api/v3/:orgSlug/pos/customers";

    // --- Shifts ---
    pub const SHIFT_SYNC: &str = "api/v3/:orgSlug/pos/shifts/sync";

    // --- Delivery ---
    pub const DRIVERS: &str = "api/v3/:orgSlug/stocking/partners";
    pub const DELIVERY_DISPATCH: &str = "api/v3/:orgSlug/stocking/reconciliation/dispatch";
    pub const DELIVERY_RECONCILE: &str = "api/v3/:orgSlug/stocking/reconciliation/reconcile-pod";

    // --- Upload ---
    pub const UPLOAD: &str = "api/v3/upload";
}
