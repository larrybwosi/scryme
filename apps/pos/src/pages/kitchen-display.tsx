...
const STATION_CONFIG: Record<Station, { label: string; color: string; icon: React.ReactNode; desc: string }> = {
  all: {
    label: 'All Stations',
    color: '#e8a020',
...
function ItemStatusDot({ status, onClick }: { status: ItemStatus; onClick: (e: React.MouseEvent) => void }) {
  const config = {
    pending: {
      label: 'Pending',
      next: 'cooking',
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <Tooltip>
        ...
        <TooltipContent side="left" className="text-[11px] bg-[#1a1d24] border-[#353a44] text-[#f0f2f5]">
          <p>
...
function OrderCard({ order, now, selected, onSelect, onStart, onBump, onVoid, onRecall, onCycleItem }: OrderCardProps) {
  ...
  const canBump = order.status === 'in_progress' || order.status === 'new' || order.status === 'urgent';
  const isDoneOrVoided = order.status === 'done' || order.status === 'voided';
  ...
  return (
    <div
      ...
      className={cn(
        ...
        selected && '!border-[#3b9eff] !border-l-4 shadow-[0_0_0_2px_rgba(59,158,255,0.25)] hover:!border-[#3b9eff]',
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-[#1d2128]">
        <span className="font-mono text-[17px] font-bold text-[#f0f2f5] leading-none tracking-tight">{order.num}</span>
        ...
        {order.status === 'urgent' && (
          <span className="flex items-center gap-1 text-[#ff4757]" title="URGENT ORDER">
              ...
              <TooltipTrigger>
                <span
                  className={cn(
                    ...
                    timerColors[level],
                    level === 'late' && 'animate-[timerFlash_0.8s_ease-in-out_infinite_alternate]'
                  )}
                >
      ...
      <div className="px-3 py-1 flex-1">
        {order.items.map((item, i) => (
          <div
            key={item.id}
            className={cn('flex items-start gap-2 py-[5px]', i < order.items.length - 1 && 'border-b border-[#1d2128]')}
          >
            <span className="font-mono text-[13px] font-bold text-[#e8a020] min-w-[20px] pt-px leading-tight">
              ×{item.quantity}
            </span>
            <div className="flex-1 min-w-0">
              <div className="text-[13px] font-semibold text-[#f0f2f5] truncate leading-tight">{item.name}</div>
              {item.modifiers && (
                <div
                  className={cn(
                ...
                >
                  {item.isAllergy && <AlertTriangle className="w-2.5 h-2.5 inline flex-shrink-0" />}
                  {item.modifiers}
                </div>
              )}
            </div>
            <TooltipProvider delayDuration={0}>
              <Tooltip>
                ...
                <TooltipContent side="left" className="text-[10px] bg-[#1a1d24] border-[#353a44] text-[#8b919e]">
      ...
      {order.note && (
        <div className="mx-3 mb-2 mt-1 px-2 py-1.5 rounded-md bg-[#e8a020]/8 border border-[#e8a020]/20 flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-[#e8a020] italic truncate">{order.note}</span>
        </div>
      )}

      {/* ── Footer / Actions ── */}
      <div className="flex items-center gap-1.5 px-3 py-2 border-t border-[#1d2128]">
        <span className="text-[10px] text-[#3a3f4a] truncate flex-1 min-w-0">{order.server}</span>
      ...
      {/* Selected indicator */}
      {selected && <div className="absolute inset-x-0 bottom-0 h-0.5 bg-[#3b9eff] rounded-b-xl" />}
    </div>
  );
}
    ...
    <Sheet open={open} onOpenChange={v => !v && onClose()}>
      ...
      >
        <SheetHeader className="px-4 py-3 border-b border-[#252830]">
          ...
          <p className="text-[11px] text-[#555b68]">Recently completed orders. Tap Recall to reopen.</p>
        </SheetHeader>
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {bumped.length === 0 ? (
            ...
            bumped.map(o => (
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="font-mono font-bold text-sm text-[#2ecc71]">{o.num}</div>
                  <div className="text-[11px] text-[#555b68]">
                    {o.table} · {o.items.length} item
                    {o.items.length !== 1 ? 's' : ''}
                  </div>
                  {o.bumpedAt && (
                    <div className="text-[10px] text-[#3a3f4a]">Cook: {formatTimer(o.bumpedAt - o.createdAt)}</div>
                  )}
                </div>
                <Button
                  size="sm"
      ...
      <DialogContent className="bg-[#111318] border border-[#252830] text-[#f0f2f5] max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-[#555b68]">
            Use these shortcuts to work faster. Select an order card first, then use the shortcut.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-2 mt-2">
          {SHORTCUTS.map(({ key, label }) => (
            <div key={key} className="flex items-center justify-between py-1.5 border-b border-[#1d2128] last:border-0">
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2 bg-[#3b9eff]/10 border-b border-[#3b9eff]/25 text-[#3b9eff] shrink-0">
      ...
      <p className="text-[11px] flex-1">
        <strong>Quick start:</strong> Click any order to select it, then tap <strong>Start</strong> →{' '}
        <strong>Bump ✓</strong> when ready. Dots on each item track cooking progress. Green = ready, orange = cooking.
      </p>
      <button onClick={onDismiss} className="text-[#3b9eff]/60 hover:text-[#3b9eff] transition-colors">
...
function StatPill({
  ...
  return (
    <div className={`flex items-center gap-1.5 text-[12px] font-semibold px-3 py-1 rounded-full border ${color}`}>
      <div className={`w-1.5 h-1.5 rounded-full ${dotColor} ${pulse ? 'animate-pulse' : ''}`} />
...
export default function KDSPage() {
  ...
  const visibleOrders = (() => {
    ...
    else if (sort === 'time_desc') list.sort((a, b) => b.createdAt - a.createdAt);
    else if (sort === 'priority') list.sort((a, b) => priorityMap[a.status] - priorityMap[b.status]);
  ...
  const stationCount = (s: Station) =>
    s === 'all'
      ? orders.filter(o => o.status !== 'done' && o.status !== 'voided').length
      : orders.filter(o => o.station === s && o.status !== 'done' && o.status !== 'voided').length;

  const avgTime = (() => {
    const recent = bumped.slice(0, 20);
    if (!recent.length) return null;
    return formatTimer(recent.reduce((a, o) => a + (o.bumpedAt! - o.createdAt), 0) / recent.length);
  })();
  ...
  const bumpOrder = useCallback(
    (id: string) => {
      ...
      if (o) {
        setBumped(prev => [{ ...o, status: 'done', bumpedAt: Date.now() } as Order, ...prev].slice(0, 50));
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === ' ' || e.key === 'Enter') {
        ...
        if (selectedId) {
          ...
          if (o && ['new', 'in_progress', 'urgent'].includes(o.status)) bumpOrder(selectedId);
        }
      }
      if (e.key === 's' || e.key === 'S') {
        if (selectedId) {
          const o = orders.find(x => x.id === selectedId);
          if (o && (o.status === 'new' || o.status === 'urgent')) startOrder(selectedId);
        }
      }
      if (e.key === 'u' || e.key === 'U') {
        if (selectedId) {
      ...
      if (['ArrowRight', 'ArrowLeft', 'ArrowDown', 'ArrowUp'].includes(e.key)) {
        ...
        const next =
          idx === -1
            ? visibleOrders[0]
            : visibleOrders[isNext ? Math.min(idx + 1, visibleOrders.length - 1) : Math.max(idx - 1, 0)];
  ...
  return (
    <>
      ...
      <div className="flex flex-col h-screen w-screen bg-[#0a0b0d] text-[#f0f2f5] overflow-hidden select-none">
        ...
        <div className="flex items-center gap-2 h-[52px] shrink-0 bg-[#111318] border-b border-[#252830] px-4 z-10">
          <div className="flex items-center gap-2 mr-3">
            ...
            <span className="text-[13px] font-bold tracking-[0.12em] uppercase text-[#e8a020]">KDS</span>
          </div>
          ...
          <div className="flex items-center gap-1.5 font-mono mr-3">
            <span className="text-[18px] font-bold text-[#f0f2f5] tracking-tight">{clockStr}</span>
            <span className="text-[11px] text-[#555b68] hidden sm:inline">{dateStr}</span>
          </div>
          ...
          {avgTime && (
            <span className="text-[12px] text-[#555b68] hidden md:flex items-center gap-1 mr-2">
              ...
              Avg: <span className="font-mono font-bold text-[#f0f2f5]">{avgTime}</span>
            </span>
          )}
        ...
        {countUrgent > 0 && (
          ...
          >
            ...
            {countUrgent} URGENT ORDER{countUrgent > 1 ? 'S' : ''} — IMMEDIATE ATTENTION REQUIRED
          </div>
        )}
        ...
        <div className="flex items-center h-11 shrink-0 bg-[#111318] border-b border-[#252830] px-3 gap-0.5 overflow-x-auto">
          {(['all', 'hot', 'cold', 'grill', 'dessert', 'bar'] as Station[]).map(s => {
              ...
              <TooltipProvider key={s} delayDuration={400}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    >
                      <span style={{ color: active ? cfg.color : undefined }} className="transition-colors">
                        {cfg.icon}
                      </span>
        ...
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0a0b0d] border-b border-[#252830] shrink-0 overflow-x-auto">
              ...
              <button
                ...
                className={cn(
                  ...
                  f === 'urgent' && filter !== f
                    ...
                    : '',
                  f === 'urgent' && filter === f ? '!text-[#ff4757] !border-[#ff4757]/40 !bg-[#ff4757]/10' : ''
                )}
              >
                {label}
              </button>
          ...
          {/* Columns */}
          <div className="flex items-center gap-1">
            <span className="text-[10px] text-[#3a3f4a] hidden sm:inline">Cols:</span>
            {[2, 3, 4, 5].map(n => (
              <button
                key={n}
                onClick={() => setCols(n)}
        ...
        {/* ── ORDER GRID ── */}
        <div className={cn('flex-1 overflow-auto p-3 grid gap-3 content-start', gridCols)}>
          {visibleOrders.length === 0 ? (
            <div className="col-span-full flex flex-col items-center justify-center py-24 text-[#555b68]">
              ...
              <p className="text-base font-bold mb-1 text-[#3a3f4a]">All Clear</p>
        ...
        <div className="flex items-center gap-3 h-10 shrink-0 bg-[#111318] border-t border-[#252830] px-4 overflow-x-auto">
          <div className="flex items-center gap-3">
            {[
              ...
              <div key={key} className="flex items-center gap-1.5 text-[10px] text-[#3a3f4a] whitespace-nowrap">
      ...
      {/* ── RECALL SHEET ── */}
      <RecallSheet open={recallOpen} bumped={bumped} onClose={() => setRecallOpen(false)} onRecall={recallOrder} />
        ...
        <AlertDialogContent className="bg-[#111318] border border-[#252830] text-[#f0f2f5] max-w-sm">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-[#555b68]">
              This will cancel order <strong className="text-[#f0f2f5]">{voidTarget?.num}</strong> for{' '}
              <strong className="text-[#f0f2f5]">{voidTarget?.table}</strong> with {voidTarget?.items.length} item
              {(voidTarget?.items.length ?? 0) > 1 ? 's' : ''}. This cannot be undone from this screen.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Dialog open={!!etaQueryTarget} onOpenChange={v => !v && setEtaQueryTarget(null)}>
        <DialogContent className="bg-[#111318] border border-[#252830] text-[#f0f2f5] max-w-sm">
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setEtaQueryTarget(null)}>
              Cancel
            </Button>
          </DialogFooter>
        </DialogContent>
