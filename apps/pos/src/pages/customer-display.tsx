...
import { MonitorSmartphone, QrCode, CreditCard, CheckCircle, DollarSign, Smartphone } from 'lucide-react';
...
const PromoSlide = ({ slide, isFullScreen = false }: { slide: any; isFullScreen?: boolean }) => (
  <div
    className={`relative z-10 p-6 flex flex-col items-center text-center ${isFullScreen ? '' : 'rounded-2xl shadow-2xl mb-6'} ${isFullScreen ? '' : slide.background}`}
  >
    {slide.type === 'qr' ? (
    ...
    <h2
      className={`${isFullScreen ? 'text-5xl md:text-7xl mb-6' : 'text-2xl lg:text-3xl mb-2'} font-bold tracking-tight ${isFullScreen ? 'text-white' : slide.textColor || 'text-white'}`}
    >
      {slide.title}
    </h2>
...
export default function CustomerDisplay() {
  ...
  const slides =
    config.promoSlides && config.promoSlides.length > 0
      ? config.promoSlides
      : [
          {
            id: 'default',
  ...
  const formatCurrency =
    utilsFormatCurrency || ((val: number) => `KSH ${val.toLocaleString(undefined, { minimumFractionDigits: 2 })}`);
                ...
                <div className="p-8 space-y-6">
                  <div className="flex justify-center">
                    <div className="p-4 border-2 border-slate-100 rounded-xl shadow-sm">
                      <QRCodeCanvas value={paymentDetails.qrData} size={220} level="H" className="rounded-lg" />
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 space-y-3">
                    <div className="flex justify-between items-baseline border-b border-slate-200 pb-2">
                      ...
                      <span className="font-bold text-slate-800">
                        {paymentDetails.mode === 'PAYBILL' ? 'Paybill' : 'Buy Goods'}
                      </span>
                    </div>
                    <div className="flex justify-between items-baseline border-b border-slate-200 pb-2">
                      <span className="text-xs uppercase font-semibold text-slate-500">Business No.</span>
                      <span className="font-mono text-lg font-bold text-slate-900">
                        {paymentDetails.mode === 'PAYBILL' ? paymentDetails.paybill : paymentDetails.tillNo}
                      </span>
                    </div>
                  ...
                  <div className="space-y-3 bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <div className="flex justify-between items-center text-lg">
                      ...
                      <span className="font-bold text-slate-900 tabular-nums">
                        {formatCurrency(paymentDetails.cashReceived || 0)}
                      </span>
                    </div>
              ...
              >
                ...
                <div className="p-10 text-center space-y-6">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 text-lg">We've sent a payment request to:</p>
                  ...
                  <div className="bg-green-50 text-green-800 p-4 rounded-xl text-sm font-medium">
                    Please enter your M-Pesa PIN to complete the payment of{' '}
                    <strong>{formatCurrency(paymentDetails.amount || 0)}</strong>
                  </div>
                </div>
              </motion.div>
            ) : null}
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs font-medium text-slate-400">
                      <span className="flex items-center gap-1">
                        <LucideIcons.Wifi size={10} /> Online
                      </span>
                      <span>•</span>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-slate-500 font-medium">
                            {item.variant ? (
                              <span className="bg-slate-100 px-1.5 py-0.5 rounded text-xs mr-2 text-slate-600">
                                {item.variant}
                              </span>
                            ) : null}
              ...
              <div className="bg-slate-950 p-6 lg:p-10 border-t border-slate-800 shadow-[0_-10px_40px_rgba(0,0,0,0.3)]">
                <div className="space-y-3 mb-6 text-sm lg:text-base font-medium">
                  ...
                  <AnimatePresence>
                    {totals.discount > 0 && (
                      ...
                      >
                        <span className="flex items-center gap-1">
                          <LucideIcons.Percent size={14} /> Savings
                        </span>
                ...
                <div className="pt-6 border-t border-dashed border-slate-800">
                  <div className="flex flex-col items-end gap-1">
                    <span className="text-xs lg:text-sm font-semibold text-slate-500 uppercase tracking-widest">
                      Total Due
                    </span>
                    <motion.span
                ...
                <div className="mt-8 flex items-center justify-between text-slate-600 text-xs font-semibold uppercase tracking-wider">
                  <span className="flex items-center gap-2">
                    <LucideIcons.Receipt size={16} /> Receipt Ready
                  </span>
                  <span className="flex items-center gap-2">
                    <MonitorSmartphone size={16} /> Terminal Active
                  </span>
                </div>
              </div>
            </aside>
