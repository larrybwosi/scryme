...
const ShiftManager: React.FC = () => {
  ...
  const handleOpenShift = async () => {
    ...
    setLoading(true);
    try {
      const shift = await shiftService.openShift(cardId, pin, openingAmount, useDetails ? denominations : null);
      setCurrentShift(shift);
      setView('STATUS');
  ...
  const AuthForm = () => (
    <div className="p-4 bg-muted/50 rounded-lg mb-6 border border-border space-y-4">
      ...
      <div className="space-y-1.5">
        <Label htmlFor="card-id" className="text-xs uppercase text-muted-foreground">
          NFC Card ID
        </Label>
      ...
      <div className="space-y-1.5">
        <Label htmlFor="pin-input" className="text-xs uppercase text-muted-foreground">
          Employee PIN
        </Label>
        <Input
          id="pin-input"
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card className="bg-orange-50/50 border-orange-100 shadow-none">
              <CardHeader className="p-4 pb-0">
                <CardDescription className="text-orange-600 font-bold text-[10px] uppercase">
                  Cash Drops
                </CardDescription>
              </CardHeader>
              <CardContent className="p-4 pt-1">
            ...
            <Card className="bg-muted/50 border-border shadow-none">
              <CardHeader className="p-4 pb-0">
                <CardDescription className="text-muted-foreground font-bold text-[10px] uppercase">
                  Starting Float
                </CardDescription>
              </CardHeader>
              <CardContent className="p-4 pt-1">
            ...
            <div className="border rounded-lg overflow-hidden">
              ...
              {showDenomDetails && (
                <div className="p-4 grid grid-cols-2 sm:grid-cols-4 gap-3 bg-background">
                  {(currentShift.opening_cash_details as CashDenomination[]).map(
                    (d: any) =>
                      d.count > 0 && (
