      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Badge
            ...
            className={cn(
              'gap-1.5 px-2 py-0.5 uppercase text-[10px] font-bold tracking-wider rounded-full cursor-help transition-all duration-300',
              config.className
            )}
          >
            {config.icon}
        ...
        <TooltipContent>
          <p className="text-xs">
            {status === 'connected'
              ? 'Successfully connected to the Local POS Hub.'
              : status === 'connecting'
                ? 'Attempting to establish connection with the Hub...'
                : status === 'error'
                  ? 'A connection error occurred. Check your network.'
