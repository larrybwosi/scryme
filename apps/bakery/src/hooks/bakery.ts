...
export const useBatches = (filters?: any) => {
  return useQuery({
    ...
    queryFn: async () => {
        ...
        const resp = await sdk.bakery.getBatches(filters) as BakeryBatchListResponse;
        return {
...
export const useCreateBatch = () => {
  ...
  return useMutation({
    mutationFn: (data: any) => {
        if (isTauri() || isOfflineMode()) {
            return tauriInvoke('create_batch', { userId: 'local-user', input: data });
        }
...
export const useCreateRecipe = () => {
  ...
  return useMutation({
    mutationFn: (data: any) => {
        if (isTauri() || isOfflineMode()) {
            return tauriInvoke('create_recipe', { userId: 'local-user', input: data });
        }
...
export const useScaleRecipe = () => {
  return useMutation({
    mutationFn: async ({ recipeId, targetYield }: { recipeId: string, targetYield: number }) => {
      if (isTauri() || isOfflineMode()) {
...
export const useBakeryData = () => {
  return useQuery({
    ...
    queryFn: async () => {
        if (isTauri() || isOfflineMode()) {
            return tauriInvoke<OverviewData>('get_overview', { org_id: 'local-org' });
        }
...
export const useUpdateRecipe = () => {
    ...
    return useMutation({
        mutationFn: ({ id, ...data }: any) => {
            if (isTauri() || isOfflineMode()) {
                return tauriInvoke('update_recipe', { userId: 'local-user', recipe: { id, ...data } });
            }
...
export const useUpdateBatch = () => {
    ...
    return useMutation({
        mutationFn: ({ id, ...data }: any) => {
            if (isTauri() || isOfflineMode()) {
                return tauriInvoke('update_batch', { userId: 'local-user', input: { id, ...data } });
            }
...
export const useStartBatch = () => {
    ...
    return useMutation({
        mutationFn: (id: string) => {
            if (isTauri() || isOfflineMode()) {
                return tauriInvoke('update_batch_status', { userId: 'local-user', id, status: 'IN_PROGRESS' });
            }
...
export const useCompleteBatch = () => {
    ...
    return useMutation({
        mutationFn: ({ batchId, data }: any) => {
            if (isTauri() || isOfflineMode()) {
                return tauriInvoke('update_batch_status', { userId: 'local-user', id: batchId, status: 'COMPLETED' });
            }
...
export const useCancelBatch = () => {
    ...
    return useMutation({
        mutationFn: (id: string) => {
            if (isTauri() || isOfflineMode()) {
                return tauriInvoke('update_batch_status', { userId: 'local-user', id, status: 'CANCELLED' });
            }
...
export const useCreateTemplate = () => {
    ...
    return useMutation({
        mutationFn: (data: any) => {
            if (isTauri() || isOfflineMode()) {
                return tauriInvoke('create_template', { userId: 'local-user', template: data });
            }
...
export const useUpdateTemplate = () => {
    ...
    return useMutation({
        mutationFn: ({ templateId, data }: any) => {
            if (isTauri() || isOfflineMode()) {
                return tauriInvoke('update_template', { userId: 'local-user', template: { id: templateId, ...data } });
            }
...
export const useCreateBakeryCategory = () => {
    ...
    return useMutation({
        mutationFn: (data: any) => {
            if (isTauri() || isOfflineMode()) {
                return tauriInvoke('create_category', { userId: 'local-user', category: data });
            }
            return sdk.bakery.createCategory(data);
        },
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['bakeryCategories'] }),
    });
};
...
export const useUpdateBakeryCategory = () => {
    ...
    return useMutation({
        mutationFn: ({ id, data }: any) => {
            if (isTauri() || isOfflineMode()) {
                return tauriInvoke('update_category', { userId: 'local-user', category: { id, ...data } });
            }
...
export const useGenerateRecipeAi = () => {
  return useMutation({
    mutationFn: async ({ prompt }: { prompt: string;}) => {
      if (isTauri() || isOfflineMode()) {
          throw new Error('AI Recipe Generation is not available in offline mode');
      }
...
export function useBakerySettingsManagement() {
  ...
  const updateSettings = useMutation({
    mutationFn: (data: any) => {
        if (isTauri() || isOfflineMode()) {
            return tauriInvoke('update_settings', { userId: 'local-user', settings: data });
        }
  ...
  const addBaker = useMutation({
    mutationFn: (data: any) => {
        if (isTauri() || isOfflineMode()) {
            return tauriInvoke('create_baker', { userId: 'local-user', baker: data });
        }
  ...
  const removeBaker = useMutation({
    mutationFn: (bakerId: string) => {
        if (isTauri() || isOfflineMode()) {
            return tauriInvoke('delete_baker', { userId: 'local-user', id: bakerId });
        }
  ...
  const updateBaker = useMutation({
    mutationFn: ({ bakerId, data }: any) => {
        if (isTauri() || isOfflineMode()) {
            return tauriInvoke('update_baker', { userId: 'local-user', baker: { id: bakerId, ...data } });
        }
