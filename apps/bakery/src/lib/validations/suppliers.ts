...
const RequiredCuidSchema = z.string({ error: 'ID is required.' }).cuid({ message: 'Invalid ID format.' });
...
export const supplierSchema = z.object({
  ...
  code: z.string().min(3, 'Supplier code must be at least 3 characters').max(20),
  ...
  businessInfo: z.object({
    taxId: z.string().min(1, 'Tax ID is required').refine((val) => {
      ...
      if (val.length === 11 && /^[A-Z]\d{9}[A-Z]$/.test(val.toUpperCase())) {
        return true;
      }
      return val.length > 0;
    }, {
