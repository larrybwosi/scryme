...
export const updateRawMaterialSchema = z.object({
  ...
  message: 'Cannot provide both baseUnitId and baseOrgUnitId in a single update.',
  path: ['baseUnitId']
});
...
export type GenerateRawMaterialFromPromptInput = z.infer<typeof generateRawMaterialFromPromptSchema>;
...
export const aiGenerationSchema = z.object({
  ...
  categoryName: z.string().min(1, 'Category name is required').transform(sanitizeString),
  ...
  baseUnitName: z.string().min(1, 'Base unit name is required (e.g., kg, item, L)'),
  initialStock: z.coerce.number().int().min(0).max(1000000).optional().nullable(),
});
