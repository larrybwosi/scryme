...
import { Check, ChevronsUpDown, Search, X, Layers, Building2 } from 'lucide-react';
          ...
          <span className="flex items-center gap-2.5 truncate min-w-0">
            {selectedUnit ? (
              <>
                ...
                <span className="text-foreground text-sm truncate">{selectedUnit.name}</span>
                <span
                  className={cn(
        ...
        <div className="flex border-b border-border/50 bg-muted/20">
          <button
            ...
            className={cn(
              ...
              tab === 'system' ? 'text-foreground' : 'text-muted-foreground hover:text-foreground/80'
            )}
          >
            ...
            <span
              className={cn(
                ...
                tab === 'system' ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'
              )}
            >
              {filteredSystemUnits.length}
            </span>
            {tab === 'system' && <span className="absolute bottom-0 left-4 right-4 h-[2px] bg-primary rounded-full" />}
          </button>
          ...
          {allowCustom && (
            <button
              ...
              className={cn(
                'relative flex-1 flex items-center justify-center gap-2 px-4 py-2.5 text-xs font-medium transition-all duration-200',
                tab === 'org' ? 'text-foreground' : 'text-muted-foreground hover:text-foreground/80'
              )}
            >
              ...
              <span
                className={cn(
                  'inline-flex items-center justify-center h-4 min-w-4 px-1 rounded-full text-[10px] font-semibold tabular-nums transition-colors',
                  tab === 'org' ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'
                )}
              >
                {filteredOrgUnits?.length || 0}
              </span>
              {tab === 'org' && <span className="absolute bottom-0 left-4 right-4 h-[2px] bg-primary rounded-full" />}
            </button>
          )}
        </div>
                ...
                {filteredSystemUnits.length === 0 ? (
                  ...
                  <ScrollArea className="h-full max-h-[260px]">
                    <div className="p-1.5 space-y-0.5">
                      {Object.entries(groupedSystemUnits).map(([type, typeUnits]) => {
                          ...
                          <UnitGroup key={type} heading={type}>
                            {filteredUnits.map(unit => (
                              <CommandItem
                                key={unit.id}
                                value={unit.id}
                                onSelect={() => handleSelect(unit.id, 'system')}
                                className={cn(
                              ...
                              >
                                ...
                                >
                                  {value === unit.id && <Check className="h-3 w-3" />}
                                </div>
                                <span className="font-mono text-xs font-semibold text-foreground/80 shrink-0 w-8">
                                  {unit.symbol}
                                </span>
                                <span className="text-foreground/90 truncate flex-1">{unit.name}</span>
                                <span
                                  className={cn(
            ...
            {tab === 'org' && allowCustom && (
              <>
                {filteredOrgUnits?.length === 0 ? (
                  ...
                  <ScrollArea className="h-full max-h-[260px]">
                    <div className="p-1.5">
                      {Object.entries(groupedOrgUnits).map(([type, typeUnits]) => {
                        const filteredUnits = typeUnits.filter(unit => {
                          if (!searchQuery) return true;
                          const query = searchQuery.toLowerCase();
                          return (
...
function UnitGroup({ heading, children }: { heading: string; children: React.ReactNode }) {
  return (
    <CommandGroup
      heading={heading}
      className={cn(
