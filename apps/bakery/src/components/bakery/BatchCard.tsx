    ...
    <Card className="group relative overflow-hidden transition-all duration-500 hover:shadow-2xl border-border/40 hover:border-primary/30 bg-card/40 backdrop-blur-md flex flex-col h-full rounded-2xl">
      ...
      <CardHeader className="pb-5 pt-7 px-7">
        <div className="flex justify-between items-start pr-10">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[11px] font-semibold text-muted-foreground/60 uppercase tracking-tight">
                {batch.recipe.name}
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 mt-4">
          <Badge
            className={`border flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest shadow-sm ${getStatusColor(batch.status)}`}
            variant="outline"
          >
          ...
          <div className="flex flex-wrap gap-1.5 mt-5">
            {batch.tags.map((tag) => (
              <Badge
                key={tag}
                variant="secondary"
                className="text-[9px] px-2 py-0.5 bg-muted/60 text-muted-foreground/80 font-bold uppercase tracking-tighter border-none rounded-md"
              >
                #{tag}
              </Badge>
            ))}
      ...
      <CardContent className="space-y-6 px-7 pb-7 flex-1 flex flex-col justify-between">
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-muted/10 p-4 rounded-2xl flex flex-col items-center justify-center text-center space-y-1.5 border border-border/20">
            <Label className="text-[9px] uppercase font-bold text-muted-foreground/50 tracking-[0.1em] cursor-default">
              Target
            </Label>
            <div className="font-bold text-sm text-foreground/80">
              {batch.plannedQuantity}{" "}
              <span className="text-[10px] opacity-60 lowercase font-medium">
                {batch.unit.symbol}
              </span>
            </div>
          </div>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    className="h-10 w-10 hover:bg-primary/5 hover:text-primary rounded-xl transition-colors"
                  >
