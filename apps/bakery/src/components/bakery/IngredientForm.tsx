...
export function IngredientFormDialog({
  ...
  const [selectedUnitType, setSelectedUnitType] = useState<"system" | "org">(
    "system",
  );

  const [selectedStockingUnitId, setSelectedStockingUnitId] = useState<
    string | undefined
  >(undefined);
  const [selectedStockingUnitType, setSelectedStockingUnitType] = useState<
    "system" | "org"
  >("system");
  ...
  const {
    ...
    CreateRawMaterialInput & { categoryId: string; tags?: string[] }
  >();
  ...
  useEffect(() => {
    if (open && ingredient) {
      reset({
        ...
        buyingPrice: ingredient.unitPrice
  ...
  const onSubmit = async (
    data: CreateRawMaterialInput & { categoryId: string },
  ) => {
    try {
      const formData = {
        ...data,
        ...
        stockingUnitId:
          selectedStockingUnitType === "system"
            ? selectedStockingUnitId
            : undefined,
        stockingOrgUnitId:
          selectedStockingUnitType === "org"
            ? selectedStockingUnitId
            : undefined,
      };
  ...
  const handleStockingUnitChange = (
    value: string | undefined,
    type: "system" | "org",
  ) => {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="buyingPrice" className="text-xs text-slate-600">
                  Standard Cost (Base)
                </Label>
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-2">
              <Scale className="h-3.5 w-3.5" /> Unit of Measure (UOM)
              Configuration
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-2 p-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 rounded-md">
                <Label className="text-xs text-slate-600">
                  Default Stocking Unit
                </Label>
                <AdvancedUnitSelector
                  value={selectedStockingUnitId || ""}
                  onValueChange={handleStockingUnitChange}
              ...
              {selectedStockingUnitId && (
                <div className="space-y-2 p-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 rounded-md sm:col-span-2">
                  ...
                  <p className="text-[10px] text-slate-400">
                    The amount of base units in one container (e.g., 25 kg in a
                    bag).
                  </p>
                </div>
              )}
