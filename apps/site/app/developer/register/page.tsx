...
import { BookText, User, Mail, Building, Lock, ArrowRight, ShieldCheck, AlertCircle } from "lucide-react";
import { colors, fonts } from "@/lib/scryme-tokens";
import { useDeveloperAuth } from "@/lib/developer-auth";

export default function DeveloperRegisterPage() {
  const router = useRouter();
  const { register, isLoading: authLoading } = useDeveloperAuth();
    ...
    >
      <div className="w-full max-w-md">
        ...
        <div className="mt-6 flex items-center justify-center gap-2 text-xs text-[rgba(241,233,216,0.5)]">
          ...
          <span>Includes free Sandbox access & test credentials</span>
        </div>
      </div>
    </div>
  );
