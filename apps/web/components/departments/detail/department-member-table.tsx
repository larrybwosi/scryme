...
export function DepartmentMemberTable({ departmentId, members }: DepartmentMemberTableProps) {
  ...
  const handleTogglePermission = async (membershipId: string, field: "canApproveExpenses" | "canManageBudget", current: boolean) => {
    setLoading(`${membershipId}-${field}`);
    const result = await updateDepartmentMember(membershipId, { [field]: !current });
    setLoading(null);
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      case "HEAD":
        return <Badge className="bg-blue-600 text-white border-none">Head</Badge>;
      case "MANAGER":
        return <Badge className="bg-purple-600 text-white border-none">Manager</Badge>;
      case "MEMBER":
    ...
    <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
      <div className="p-4 border-b flex items-center justify-between bg-gray-50/30">
        <div>
          ...
          <p className="text-xs text-gray-500">Manage who has access to this department and their roles.</p>
        </div>
        <AddMemberToDeptSheet departmentId={departmentId}>
          <Button size="sm" className="gap-2 bg-[#1D1D1F] text-white">
            <UserPlus size={14} />
            <span>Add Member</span>
          </Button>
        </AddMemberToDeptSheet>
      </div>
      <Table>
        <TableHeader>
          <TableRow>
              ...
              <TableRow key={membership.id}>
                <TableCell>
                  <div className="flex items-center gap-2">
                    <Avatar className="h-8 w-8 border">
                      ...
                      <AvatarFallback>{membership.member.user.name.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div className="flex flex-col">
                      <span className="text-sm font-medium">{membership.member.user.name}</span>
                      <span className="text-[10px] text-gray-500">{membership.member.user.email}</span>
                    </div>
                  </div>
                </TableCell>
                ...
                <TableCell>
                  <Button
                    ...
                    onClick={() => handleTogglePermission(membership.id, "canApproveExpenses", membership.canApproveExpenses)}
                    disabled={loading === `${membership.id}-canApproveExpenses`}
                  >
                    <ShieldCheck size={14} />
                    <span className="text-[11px]">{membership.canApproveExpenses ? "Enabled" : "Disabled"}</span>
                  </Button>
                </TableCell>
                <TableCell>
                  <Button
                    ...
                    onClick={() => handleTogglePermission(membership.id, "canManageBudget", membership.canManageBudget)}
                    disabled={loading === `${membership.id}-canManageBudget`}
                  >
                    <CreditCard size={14} />
                    <span className="text-[11px]">{membership.canManageBudget ? "Enabled" : "Disabled"}</span>
                  </Button>
                </TableCell>
