...
import { MoreHorizontal, Eye, Edit, Trash2, Building2, Star } from "lucide-react";
          ...
          {data.length === 0 ? (
            ...
            data.map((supplier) => (
              <TableRow
                ...
                onClick={() => router.push(`/inventory/supplier/${supplier.id}`)}
              >
                <TableCell>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="font-bold text-sm text-[#1D1D1F]">{supplier.name}</span>
                      <div className="flex items-center gap-1 text-[11px] text-muted-foreground">
                        <Star size={10} className="text-yellow-500 fill-yellow-500" />
                        <span>{supplier.avgRating} ({supplier.reviewCount})</span>
                      </div>
                    </div>
                  </div>
                </TableCell>
                ...
                <TableCell>
                  <span className="text-sm font-medium">{supplier._count?.products || 0}</span>
                </TableCell>
                <TableCell>
                  <span className="text-sm font-medium">{supplier._count?.purchases || 0}</span>
                </TableCell>
                <TableCell>
                  <Badge
                    variant="secondary"
                    className={cn(
                      ...
                      supplier.riskLevel === "high" && "bg-red-100 text-red-700"
                    )}
                  >
                    {supplier.riskLevel}
                  </Badge>
