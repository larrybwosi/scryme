      ...
      <Dialog open={isBlockDialogOpen} onOpenChange={setIsBlockDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to block <strong>{member.user.name || member.user.email}</strong>?
              They will not be able to access the organization until unblocked.
            </DialogDescription>
          </DialogHeader>
      ...
      <Sheet open={isSessionsSheetOpen} onOpenChange={setIsSessionsSheetOpen}>
        <SheetContent className="sm:max-w-[540px]">
          <SheetHeader>
            ...
            <SheetDescription>
              Manage active login sessions for {member.user.name || member.user.email}.
            </SheetDescription>
          </SheetHeader>
          <div className="py-6 space-y-4">
            {sessions.length === 0 ? (
              <p className="text-center text-gray-500 py-10">No active sessions found.</p>
            ) : (
              sessions.map((session) => (
                <div key={session.id} className="flex items-center justify-between p-4 border rounded-xl bg-gray-50/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">
                        IP: {session.ipAddress || "Unknown"} • Last active: {format(new Date(session.updatedAt), "MMM d, HH:mm")}
                      </div>
                    </div>
                  </div>
                  <Button
      ...
      <Sheet open={isRolesSheetOpen} onOpenChange={setIsRolesSheetOpen}>
        <SheetContent className="sm:max-w-[450px]">
          <SheetHeader>
            ...
            <SheetDescription>
              Assign custom roles to {member.user.name || member.user.email} to grant specific permissions.
            </SheetDescription>
          </SheetHeader>
          <div className="py-6 space-y-4">
            {customRoles.length === 0 ? (
              <div className="text-center py-10 border-2 border-dashed rounded-xl">
                ...
                <p className="text-sm text-gray-500">No custom roles defined for this organization.</p>
                <Button variant="link" className="text-[#34A853]">Create one now</Button>
              </div>
            ) : (
                ...
                {customRoles.map((role) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <Label className="font-semibold cursor-pointer">{role.name}</Label>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 bg-white border-t">
            <Button className="w-full" onClick={handleUpdateRoles} disabled={loading || customRoles.length === 0}>
