...
export function AuditStockModal({ isOpen, onClose, product }: AuditStockModalProps) {
  const [activeTab, setActiveTab] = useState("quantity");
  const [physicalCount, setPhysicalCount] = useState<string>(product.currentStock.toString());
  const [reason, setReason] = useState<StockAdjustmentReason>("INVENTORY_COUNT" as StockAdjustmentReason);
  const [notes, setNotes] = useState("");
  const [newValue, setNewValue] = useState<string>(product.unitPrice.toString());
  ...
  const reasons: { label: string; value: StockAdjustmentReason; icon: any }[] = [
    { label: "Damage", value: "DAMAGED" as StockAdjustmentReason, icon: AlertTriangle },
    { label: "Expire", value: "EXPIRED" as StockAdjustmentReason, icon: Clock },
    { label: "Misplacement", value: "LOST" as StockAdjustmentReason, icon: Info },
    { label: "Thief", value: "STOLEN" as StockAdjustmentReason, icon: ShieldAlert },
    { label: "Stocktake Variance", value: "INVENTORY_COUNT" as StockAdjustmentReason, icon: Tag },
    { label: "Custom", value: "OTHER" as StockAdjustmentReason, icon: Package },
  ];
        ...
        <div className="flex h-full min-h-[600px]">
          ...
          <div className="flex-1 p-6 border-r">
            <DialogHeader className="mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <DialogTitle className="text-xl">Audit Stock</DialogTitle>
                  <p className="text-sm text-gray-500">Effortlessly import products and update your inventory.</p>
                </div>
              </div>
            </DialogHeader>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
              <TabsList className="grid w-full grid-cols-2 bg-gray-50 p-1">
                <TabsTrigger value="quantity" className="data-[state=active]:bg-white data-[state=active]:shadow-sm">
                  Quantity Adjustment
                </TabsTrigger>
                <TabsTrigger value="value" className="data-[state=active]:bg-white data-[state=active]:shadow-sm">
                  Value Adjustment
                </TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="quantity" className="space-y-6 mt-0">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-2">
                    <Label className="text-xs text-gray-500">Physical Count</Label>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Input value={availableAtLocation} readOnly className="bg-gray-50 pr-12" />
                      <span className="absolute right-3 top-2.5 text-xs text-gray-400">Unit</span>
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Input value={discrepancy} readOnly className="bg-gray-50 pr-12 font-medium text-orange-600" />
                      <span className="absolute right-3 top-2.5 text-xs text-gray-400">Unit</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {reasons.map((r) => (
                      ...
                      >
                        ...
                        {reason === r.value && <div className="w-2 h-2 rounded-full bg-black" />}
                        {reason !== r.value && <div className="w-2 h-2 rounded-full border border-gray-300" />}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <TabsContent value="value" className="space-y-6 mt-0">
                <div className="space-y-2">
                  <Label className="text-xs text-gray-500">New Unit Price</Label>
                  <div className="relative">
                    <Input
                      value={newValue}
            ...
            <div className="flex items-center justify-between mt-8 pt-6 border-t">
              <a href="#" className="text-xs text-blue-600 hover:underline">Learn more about Audit Stock</a>
              <div className="flex gap-2">
                <Button variant="outline" onClick={onClose}>Cancel</Button>
                <Button className="bg-orange-500 hover:bg-orange-600 text-white min-w-[80px]" onClick={handleSave} disabled={isSubmitting}>
          ...
          <div className="w-[300px] bg-gray-50/50 p-6 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto">
              <h3 className="text-sm font-bold mb-4">Stock adjustment history</h3>
                ...
                {history.map((item, idx) => (
                  <div key={item.id} className="relative pl-8">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <span className="text-[10px] text-gray-600">{item.member?.user?.name || "Unknown"}</span>
                    </div>
                  </div>
                ))}
                {history.length === 0 && (
                  <div className="text-xs text-gray-500 pl-8">No history available</div>
                )}
              </div>
            </div>
          </div>
