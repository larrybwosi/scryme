            ...
            <div className="space-y-6 relative before:absolute before:left-2 before:top-2 before:bottom-2 before:w-[1px] before:bg-gray-200">
              {history.map((item) => (
                <div key={item.id} className="relative pl-8">
                  ...
                  <div className="text-sm font-bold text-gray-800">{format(new Date(item.adjustmentDate), "dd MMM yyyy, HH:mm")}</div>
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    ...
                    <span className="text-xs text-gray-600">{item.member?.user?.name || "Unknown"}</span>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <div className="text-center py-10 text-gray-500">No stock history found for this product.</div>
          )}
        </div>
      </SheetContent>
    </Sheet>
