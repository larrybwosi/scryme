...
export function RevenueChart({ data, totalValue, change, periodLabel }: RevenueChartProps) {
        ...
        <ChartContainer config={chartConfig}>
          <LineChart data={data}>
            ...
            <XAxis
              ...
              ticks={data.length > 0 ? [data[0]?.date, data[Math.floor(data.length/2)]?.date, data[data.length-1]?.date] : []}
            />
            <YAxis
               axisLine={false}
               tickLine={false}
