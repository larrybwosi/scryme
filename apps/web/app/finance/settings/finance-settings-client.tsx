...
type SectionId = "general" | "workflows" | "cost-centers" | "categories" | "budget-alerts";

const SECTIONS = [
  { id: "general", label: "General Settings", icon: Settings, description: "Thresholds and global flags" },
  { id: "workflows", label: "Approval Workflows", icon: GitMerge, description: "Multi-step approval processes" },
  { id: "cost-centers", label: "Cost Centers", icon: Building, description: "Dimensional reporting and tracking" },
  { id: "categories", label: "Expense Categories", icon: Tag, description: "Categorization and GL mapping" },
  { id: "budget-alerts", label: "Budget Alerts", icon: Bell, description: "Spend monitoring and notifications" },
];
...
export function FinanceSettingsClient({ initialData }: { initialData: any }) {
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case "cost-centers":
        return <CostCenterManager initialCostCenters={initialData.costCenters} />;
      case "categories":
            ...
            return (
              ...
              >
                <div className={cn(
                  ...
                  isActive ? "bg-[#34A853] text-white" : "bg-zinc-100 text-zinc-500 group-hover:bg-zinc-200"
                )}>
                ...
                {isActive && <ChevronRight className="w-4 h-4 text-zinc-300 self-center" />}
              </button>
            );
          })}
        </nav>
        ...
        <div className="mt-8 p-4 rounded-xl bg-zinc-900 text-white overflow-hidden relative">
          <div className="relative z-10">
            ...
            <p className="text-[11px] text-zinc-400 mt-2 leading-relaxed">
              All financial changes are audited and logged. Multi-admin approval is required for sensitive workflow changes.
            </p>
          </div>
