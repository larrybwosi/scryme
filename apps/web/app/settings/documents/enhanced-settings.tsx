...
export function EnhancedDocumentSettings({
  ...
  const documentTypes = [
    {
      ...
      defaultTemplateId: organization.settings?.defaultInvoiceTemplate || "invoice-v2",
    },
    {
      ...
      defaultTemplateId: organization.settings?.defaultReceiptTemplate || "receipt-v2",
    },
    {
      ...
      defaultTemplateId: organization.settings?.defaultWaybillTemplate || "waybill-default",
    },
  ];
  ...
  const handleUpdateDefault = async (templateId: string) => {
    ...
    try {
      ...
      organization.settings = {
        ...organization.settings,
        [`default${selectedType.charAt(0) + selectedType.slice(1).toLowerCase()}Template`]: templateId,
      };
    } catch (error) {
  ...
  const getMockDataForType = (type: DocumentType) => {
    ...
    if (mockData.branding) {
      mockData.branding.primaryColor = invoiceConfig?.primaryColor || mockData.branding.primaryColor;
  ...
  const renderCategories = () => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {documentTypes.map((type) => (
        ...
        >
          <div className={cn("w-12 h-12 rounded-lg flex items-center justify-center mb-4 transition-colors", type.color)}>
            <type.icon size={24} />
          </div>
  ...
  const renderTemplates = () => {
    ...
    const activeTemplateId = documentTypes.find((t) => t.id === selectedType)
      ?.defaultTemplateId;
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <Dialog open={!!previewTemplate} onOpenChange={() => setPreviewTemplate(null)}>
        <DialogContent className="max-w-5xl h-[90vh] flex flex-col p-0 overflow-hidden bg-gray-100">
          <DialogHeader className="p-4 bg-white border-b flex-shrink-0">
            <div className="flex items-center justify-between pr-8">
              <div>
                ...
                <p className="text-sm text-gray-500">Document Preview • {previewTemplate?.version}</p>
              </div>
