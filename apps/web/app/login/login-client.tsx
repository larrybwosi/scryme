...
function LoginPageContent({
    ...
    <div className="relative pl-1">
      {VERIFICATION_STEPS.map((step, i) => (
        <div
          ...
          <span className={cn(sansClass, "text-sm text-white/85 flex-1")}>
            {step.label}
          </span>
          <span
            className={cn(
  ...
  const StatCard = ({ value, label }: { value: string; label: string }) => (
    <div className="flex flex-col">
      <span className={cn(displayClass, "text-2xl font-medium text-white")}>
        {value}
      </span>
      <span
        className={cn(
  ...
  return (
    <div className="min-h-screen flex bg-[#FBFAF7] w-full">
      {/* ── Left Panel ── */}
      <div className="flex-1 flex items-center justify-center p-8 lg:p-12">
        <div className="w-full max-w-[420px]">
          ...
          <div className="mt-6 flex items-center justify-center gap-2 text-[11px] text-[#9AA6B2]">
            ...
            <span className={cn(monoClass, "tracking-wide")}>
...
export function LoginPageClient(props: LoginClientProps) {
  return (
    <div className={cn(props.displayVar, props.sansVar, props.monoVar, "w-full min-h-screen")}>
      <Suspense
        fallback={
