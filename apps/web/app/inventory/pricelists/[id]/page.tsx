      ...
      <div className="flex flex-col gap-4">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink href="/inventory/pricelists">
                Price Lists
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
        ...
        <div className="grid grid-cols-3 gap-4 mt-2">
          ...
          <div className="bg-white p-3 rounded-lg border flex flex-col gap-1">
            ...
            <span className="text-sm text-gray-600">
              Currency: <span className="font-bold">{priceList.currency}</span>{" "}
              • Priority:{" "}
              <span className="font-bold">{priceList.priority}</span>
            </span>
          </div>
        </div>
      </div>
      ...
      <Tabs defaultValue="items" className="w-full">
        <TabsList className="bg-white border mb-4">
          <TabsTrigger value="items">
            Price List Items ({priceList.items.length})
          </TabsTrigger>
          <TabsTrigger value="rules">
            Pricing Rules ({priceList.rules.length})
          </TabsTrigger>
        ...
        <TabsContent value="customers" className="mt-0">
          <PriceListCustomerTable
            priceListId={priceList.id}
            customers={priceList.customers}
          />
        </TabsContent>
      </Tabs>
    </div>
