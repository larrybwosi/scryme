          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl border shadow-sm space-y-4">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider text-muted-foreground">
                    Description
                  </label>
                      ...
                      {dept.head ? (
                        <>
                          <Avatar className="h-6 w-6">
                            <AvatarImage
                              src={dept.head.user.image ?? undefined}
                            />
                            <AvatarFallback>
                  ...
                  <div className="p-4 bg-green-50/50 rounded-xl border border-green-100">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-medium text-green-600">
                        Active: {dept.activeBudget.name}
                      </span>
                      <Badge className="bg-green-600 text-white border-none text-[10px]">
                        IN PROGRESS
                      </Badge>
                    </div>
                    ...
                    <div className="text-[10px] text-green-600 mt-1">
                      Ends on{" "}
                      {format(
                        new Date(dept.activeBudget.periodEnd),
                        "MMM dd, yyyy",
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-bold">
                        {Math.round(
                          (Number(dept.activeBudget.spentAmount) /
                            Number(dept.activeBudget.amount)) *
                            100,
                        ) || 0}
                        %
                      </span>
                    ...
                    <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(Math.round((Number(dept.activeBudget.spentAmount) / Number(dept.activeBudget.amount)) * 100), 100) || 0}%`,
                        }}
                      />
                    </div>
                    <p className="text-[10px] text-gray-500">
                      Spent: KES{" "}
                      {Number(dept.activeBudget.spentAmount).toLocaleString()} /
                      Total: KES{" "}
                      {Number(dept.activeBudget.amount).toLocaleString()}
                    </p>
                  </div>
                </div>
              ) : (
          ...
          <div className="lg:col-span-2">
            <Tabs defaultValue="members" className="w-full">
              <TabsList className="bg-white border p-1 rounded-xl w-full justify-start h-auto mb-6 overflow-x-auto">
                <TabsTrigger
                  value="members"
                  className="gap-2 px-6 py-2.5 rounded-lg data-[state=active]:bg-gray-100 data-[state=active]:text-[#1D1D1F]">
                  <Users size={16} />
                  <span>Members</span>
                </TabsTrigger>
                <TabsTrigger
