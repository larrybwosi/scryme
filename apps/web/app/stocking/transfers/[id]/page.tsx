...
import { getStockTransferDetails, updateStockTransferStatus } from "../../../actions/stock-management";
    ...
    <div className="flex flex-col gap-6 p-8 bg-gray-50/50 min-h-screen">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500">Requested on {format(new Date(transfer.requestedDate), "MMMM dd, yyyy")}</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {(transfer.status === "PENDING_APPROVAL" || transfer.status === "APPROVED") && (
            <form action={handleAction}>
              ...
              <Button type="submit" variant="outline" className="gap-2 text-red-600 hover:text-red-700">
                <XCircle size={16} />
                Reject
              </Button>
            </form>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <TableBody>
                  {transfer.items.map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="text-right font-medium">{item.requestedQuantity.toNumber()}</TableCell>
                      <TableCell className="text-right">${item.unitCost.toNumber().toFixed(2)}</TableCell>
                      <TableCell className="text-right font-medium">
                        ${(item.requestedQuantity.toNumber() * item.unitCost.toNumber()).toFixed(2)}
                      </TableCell>
                    </TableRow>
                  ))}
            ...
            <CardContent className="space-y-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-500">By {transfer.requestedBy.user.name} on {format(new Date(transfer.requestedDate), "PPP p")}</div>
                </div>
              </div>
              ...
              {transfer.shippedDate && (
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500">By {transfer.shippedBy?.user.name} on {format(new Date(transfer.shippedDate), "PPP p")}</div>
                  </div>
                </div>
              )}
              ...
              {transfer.completedDate && (
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500">By {transfer.receivedBy?.user.name} on {format(new Date(transfer.completedDate), "PPP p")}</div>
                  </div>
                </div>
              )}
            </CardContent>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-sm uppercase tracking-wider text-gray-500">Route Info</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="flex items-start gap-3">
                <MapPin className="text-gray-400 mt-1" size={18} />
                <div>
                  <div className="text-xs text-gray-400 uppercase font-bold">From Location</div>
                  <div className="font-bold text-gray-900">{transfer.fromLocation.name}</div>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <MapPin className="text-gray-400 mt-1" size={18} />
                <div>
                  <div className="text-xs text-gray-400 uppercase font-bold">To Location</div>
                  <div className="font-bold text-gray-900">{transfer.toLocation.name}</div>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-sm uppercase tracking-wider text-gray-500">Notes</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-start gap-3">
                <FileText className="text-gray-400 mt-1" size={18} />
                <p className="text-sm text-gray-600">{transfer.notes || "No notes provided."}</p>
              </div>
            </CardContent>
          </Card>
        </div>
