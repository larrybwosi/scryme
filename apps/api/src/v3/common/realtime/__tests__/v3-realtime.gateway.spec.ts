...
describe("V3RealtimeGateway", () => {
  ...
  it("should allow publishing to an authorized order channel", async () => {
    ...
    vi.mocked(prisma.client.transaction.findUnique).mockResolvedValue({
      organizationId: "org-1",
    } as any);

    const data = {
      channel: "order:order-1",
      event: "status",
      data: { status: "PAID" },
    };
    const result = await gateway.handlePublish(client, data);

    expect(redis.saveMessage).toHaveBeenCalledWith("order:order-1", "status", {
      status: "PAID",
    });
