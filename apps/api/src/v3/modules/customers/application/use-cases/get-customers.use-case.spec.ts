...
describe('GetCustomersUseCase', () => {
  ...
  it('should return customers for a given organization', async () => {
    const orgId = 'org-1';
    const customers = [new Customer('1', 'Cust 1', 'cust1@example.com', null, orgId, new Date(), new Date())];
    ...
    (customerRepository.findByOrganization as any).mockResolvedValue(paginatedResponse);

    const result = await useCase.execute(orgId, { limit: 20, offset: 0 });

    expect(result).toEqual(paginatedResponse);
    expect(customerRepository.findByOrganization).toHaveBeenCalledWith(orgId, { limit: 20, offset: 0 });
  });
});
