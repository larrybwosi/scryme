...
export class RegisterCustomerUseCase {
  ...
  async execute(organizationId: string, dto: RegisterCustomerDto) {
    this.logger.log(`Registering customer for organization ${organizationId}: ${dto.email}`);

    await this.verifyZitadelUser(dto.zitadelUserId);

    const result = await this.prisma.client.$transaction(async tx => {
      const internalId = await this.getOrCreateInternalMapping(tx, organizationId, dto.zitadelUserId);
      const customer = await this.upsertCustomer(tx, internalId, organizationId, dto);
    ...
    return new Customer(result.id, result.name, result.email, null, result.organizationId, new Date(), new Date());
  }
  ...
  private async verifyZitadelUser(zitadelUserId: string) {
    ...
    try {
      ...
      if (!user) throw new NotFoundException(`Zitadel user with ID ${zitadelUserId} not found`);
    } catch (e) {
      throw new NotFoundException(`Zitadel user with ID ${zitadelUserId} not found`);
    }
  }

  private async getOrCreateInternalMapping(tx: any, organizationId: string, zitadelUserId: string) {
    const mapping = await tx.externalMapping.findFirst({
      where: { organizationId, provider: 'ZITADEL', externalId: zitadelUserId, entityType: 'CUSTOMER' },
    });
    ...
    await tx.externalMapping.create({
      data: { organizationId, internalId, externalId: zitadelUserId, provider: 'ZITADEL', entityType: 'CUSTOMER' },
    });
    return internalId;
  }

  private async upsertCustomer(tx: any, internalId: string, organizationId: string, dto: RegisterCustomerDto) {
    const customerData = {
      name: dto.name,
      email: dto.email,
      phone: dto.phone,
  ...
  private async handleStructuredAddress(tx: any, customerId: string, addressDto: any) {
    const addressData = {
      type: 'BOTH' as const,
    ...
    const existingAddress = await tx.address.findFirst({
      where: { customerId, street1: addressDto.street1, city: addressDto.city, postalCode: addressDto.postalCode },
    });
  ...
  private async triggerExternalAutomations(
    ...
    try {
      ...
      this.logger.error(`Failed to trigger external automations: ${e instanceof Error ? e.message : String(e)}`);
    }
  }
}
