...
export class BusinessAccountController {
  ...
  async create(@Request() req: any, @Body() body: any) {
    return this.businessAccountService.createBusinessAccount(req.organization.id, body);
  }

  @Get(':id')
  @ApiOperation({ summary: 'Get a business account with CRM timeline' })
  async getOne(@Request() req: any, @Param('id') id: string) {
    return this.businessAccountService.getBusinessAccount(req.organization.id, id);
  }
}
