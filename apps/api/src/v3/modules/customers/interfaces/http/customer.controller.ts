import { Controller, Get, Post, Body, UseGuards, UseInterceptors, Req, Query, Param, Patch } from '@nestjs/common';
...
export class CustomerController {
  ...
  @ApiResponse({ status: 200, type: [CustomerResponseDto], description: 'List of customers' })
  @ApiResponse({ status: 401, type: ApiErrorResponseDto, description: 'Unauthorized' })
  async getCustomers(@Req() req: any, @Query() paginationQuery: PaginationQueryDto) {
    return this.getCustomersUseCase.execute(req.organization.id, paginationQuery);
  }
  ...
  @ApiResponse({ status: 201, type: CustomerResponseDto, description: 'Customer registered successfully' })
  ...
  @ApiResponse({ status: 400, type: ApiErrorResponseDto, description: 'Invalid input' })
  @ApiResponse({ status: 401, type: ApiErrorResponseDto, description: 'Unauthorized' })
  @ApiResponse({ status: 404, type: ApiErrorResponseDto, description: 'Customer not found' })
