...
export class MembersController {
  ...
  async createMember(@Request() req: any, @Body() dto: CreateMemberDto) {
    ...
    return this.memberUseCase.createMember(req.v3Context.organizationId, dto, inviterId);
  }
  ...
  @ApiResponse({ status: 200, type: MemberResponseDto })
  async updateMember(@Request() req: any, @Param('id') id: string, @Body() dto: UpdateMemberDto) {
    const actorId = req.v3Context.memberId;
    return this.memberUseCase.updateMember(req.v3Context.organizationId, id, dto, actorId);
  }
  ...
  async deleteMember(@Request() req: any, @Param('id') id: string) {
    const actorId = req.v3Context.memberId;
    return this.memberUseCase.deleteMember(req.v3Context.organizationId, id, actorId);
  }

  @Get(':id/activity')
  @Permissions('members:read')
  @ApiOperation({ summary: 'Get audit logs for a specific member' })
  async getMemberActivity(@Request() req: any, @Param('id') id: string, @Query() query: any) {
    return this.memberUseCase.getMemberAuditLogs(req.v3Context.organizationId, id, query);
  }
