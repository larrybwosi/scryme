import { Controller, Get, Post, Delete, Body, Param, Query, UseGuards, Request, UseInterceptors, UnauthorizedException } from '@nestjs/common';
...
import { CreateInvitationDto, InvitationQueryDto, InvitationResponseDto, AcceptInvitationDto } from '../../application/dto/invitation.dto';
...
export class InvitationController {
  ...
  async getInvitations(@Request() req: any, @Query() query: InvitationQueryDto) {
    return this.invitationUseCase.getInvitations(req.v3Context.organizationId, query);
  }
  ...
  async createInvitation(@Request() req: any, @Body() dto: CreateInvitationDto) {
    ...
    if (!inviterUserId) throw new UnauthorizedException('Inviter user ID not found');

    return this.invitationUseCase.createInvitation(req.v3Context.organizationId, dto, inviterId, inviterUserId);
  }
  ...
  async revokeInvitation(@Request() req: any, @Param('id') id: string) {
    ...
    return this.invitationUseCase.revokeInvitation(req.v3Context.organizationId, id, actorId);
  }
