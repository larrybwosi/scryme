...
import { CheckInDto, CheckOutDto, AttendanceQueryDto } from '../../application/dto/attendance.dto';
...
export class AttendanceController {
  ...
  async getLogs(@Request() req: any, @Query() query: AttendanceQueryDto) {
    return this.attendanceUseCase.getAttendanceLogs(req.v3Context.organizationId, query);
  }
  ...
  async checkIn(@Request() req: any, @Body() dto: CheckInDto) {
    ...
    return this.attendanceUseCase.checkIn(req.v3Context.organizationId, memberId, dto);
  }
  ...
  async checkOut(@Request() req: any, @Body() dto: CheckOutDto) {
    ...
    return this.attendanceUseCase.checkOut(req.v3Context.organizationId, memberId, dto);
  }
  ...
  async getStatus(@Request() req: any, @Param('memberId') memberId: string) {
    return this.attendanceUseCase.getMemberStatus(req.v3Context.organizationId, memberId);
  }
}
