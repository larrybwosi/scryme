...
export class RoleManagementController {
  ...
  async getCustomRoles(@Request() req: any, @Query() query: CustomRoleQueryDto) {
  ...
  async createCustomRole(@Request() req: any, @Body() dto: CreateCustomRoleDto) {
    const actorId = req.v3Context.memberId;
    return this.roleUseCase.createCustomRole(req.v3Context.organizationId, dto, actorId);
  }

  @Patch('custom/:id')
  @Permissions('roles:write')
  @ApiOperation({ summary: 'Update a custom role' })
  async updateCustomRole(@Request() req: any, @Param('id') id: string, @Body() dto: UpdateCustomRoleDto) {
    const actorId = req.v3Context.memberId;
    return this.roleUseCase.updateCustomRole(req.v3Context.organizationId, id, dto, actorId);
  }
  ...
  async deleteCustomRole(@Request() req: any, @Param('id') id: string) {
    const actorId = req.v3Context.memberId;
    return this.roleUseCase.deleteCustomRole(req.v3Context.organizationId, id, actorId);
  }
  ...
  async createPermissionSet(@Request() req: any, @Body() dto: CreatePermissionSetDto) {
    const actorId = req.v3Context.memberId;
    return this.roleUseCase.createPermissionSet(req.v3Context.organizationId, dto, actorId);
  }
  ...
  async createRoleGroup(@Request() req: any, @Body() dto: CreateRoleGroupDto) {
    const actorId = req.v3Context.memberId;
    return this.roleUseCase.createRoleGroup(req.v3Context.organizationId, dto, actorId);
  }
  ...
  async assignRoles(@Request() req: any, @Param('memberId') memberId: string, @Body('roleIds') roleIds: string[]) {
    const actorId = req.v3Context.memberId;
    return this.roleUseCase.assignRolesToMember(req.v3Context.organizationId, memberId, roleIds, actorId);
  }

  @Post('member/:memberId/remove')
  @Permissions('members:write')
  @ApiOperation({ summary: 'Remove custom roles from a member' })
  async removeRoles(@Request() req: any, @Param('memberId') memberId: string, @Body('roleIds') roleIds: string[]) {
    const actorId = req.v3Context.memberId;
    return this.roleUseCase.removeRolesFromMember(req.v3Context.organizationId, memberId, roleIds, actorId);
  }
}
