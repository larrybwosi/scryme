...
describe("MemberUseCase", () => {
  ...
  describe("Security Enhancements", () => {
    ...
    describe("login (Terminal)", () => {
      ...
      it("should call bcrypt.compare with dummy pin hash and fail if member is not found (mitigates timing attack)", async () => {
        const orgId = "org1";
        ...
        expect(bcrypt.compare).toHaveBeenCalledWith(pin, expectedDummyHash);
        expect(mockRedis.incr).toHaveBeenCalled();
      });
    });
  });
