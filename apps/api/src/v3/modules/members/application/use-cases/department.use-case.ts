...
export class DepartmentUseCase {
  ...
  async addMemberToDepartment(organizationId: string, departmentId: string, dto: AddDepartmentMemberDto, actorId: string) {
    // Verify department and member belong to org
    const [dept, member] = await Promise.all([
      this.prisma.client.department.findFirst({ where: { id: departmentId, organizationId } }),
      this.prisma.client.member.findFirst({ where: { id: dto.memberId, organizationId } }),
    ]);
  ...
  async removeMemberFromDepartment(organizationId: string, departmentId: string, memberId: string, actorId: string) {
