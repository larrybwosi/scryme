import { Controller, Get, Post, Delete, Body, Query, UseGuards, Request, UseInterceptors } from '@nestjs/common';
import { ApiTags, ApiOperation, ApiBearerAuth, ApiResponse } from '@nestjs/swagger';
import { V3AuthGuard } from '@/v3/common/guards/v3-auth.guard';
import { MultiTenancyGuard } from '@/v3/common/guards/multi-tenancy.guard';
import { StandardResponseInterceptor } from '@/v3/common/interceptors/standard-response.interceptor';
import { CartUseCase } from '../../application/use-cases/cart.use-case';
import { AddToCartDto, RemoveFromCartDto, CartResponseDto } from '../../application/dto/cart.dto';
...
export class CartController {
  ...
  async addToCart(@Request() req: any, @Body() dto: AddToCartDto) {
    ...
    return this.cartUseCase.addToCart(req.organization.id, { ...dto, customerId: customerId || dto.customerId });
  }
  ...
  async removeFromCart(@Request() req: any, @Body() dto: RemoveFromCartDto) {
    const customerId = req.user?.id;
    return this.cartUseCase.removeFromCart(req.organization.id, { ...dto, customerId: customerId || dto.customerId });
  }
}
