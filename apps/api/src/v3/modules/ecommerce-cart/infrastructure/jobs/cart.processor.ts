...
export class CartProcessor extends WorkerHost {
  ...
  private async checkInventory(data: { cartId: string; productId: string; organizationId: string }) {
    this.logger.log(`Checking inventory for cart ${data.cartId}, product ${data.productId}`);
