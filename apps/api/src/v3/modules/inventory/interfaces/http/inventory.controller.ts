import { Controller, Get, Post, Patch, Body, Param, UseGuards, UseInterceptors, Req, Query } from '@nestjs/common';
...
export class InventoryController {
  ...
  @ApiOperation({ summary: 'Verify inventory integrity', operationId: 'Inventory_VerifyIntegrity' })
  async verifyIntegrity(@Req() req: any) {
    return this.integrityService.verifyOrganizationIntegrity(req.organization.id);
  }

  @Post('integrity/fix/:variantId')
  @Permissions('inventory:manage')
  @ApiOperation({ summary: 'Fix inventory integrity for a variant', operationId: 'Inventory_FixIntegrity' })
  async fixIntegrity(@Req() req: any, @Param('variantId') variantId: string, @Query('locationId') locationId: string) {
    return this.integrityService.fixVariantIntegrity(req.organization.id, variantId, locationId);
  }
  ...
  @ApiResponse({ status: 200, type: [InventoryResponseDto], description: 'Inventory list' })
  @ApiResponse({ status: 401, type: ApiErrorResponseDto, description: 'Unauthorized' })
  async getInventory(@Req() req: any, @Query() paginationQuery: PaginationQueryDto) {
    return this.getInventoryUseCase.execute(req.organization.id, paginationQuery);
  }

  @Get('trace/:identifier')
  @Permissions('inventory:read')
  @ApiOperation({ summary: 'Trace a batch by ID or number', operationId: 'Inventory_TraceBatch' })
  ...
  @ApiOperation({ summary: 'Split a stock batch into child batches', operationId: 'Inventory_SplitBatch' })
  async splitBatch(
    @Req() req: any,
    @Param('id') id: string,
    @Body() body: { splits: { quantity: number; notes?: string }[] }
  ) {
    return this.splitBatchUseCase.execute(req.organization.id, id, req.user.memberId, body.splits);
  }

  @Post('batches/merge')
  @Permissions('inventory:write')
  @ApiOperation({ summary: 'Merge multiple batches into one', operationId: 'Inventory_MergeBatches' })
  async mergeBatches(@Req() req: any, @Body() body: { batchIds: string[]; targetLocationId: string; notes?: string }) {
  ...
  @Post('assemblies')
  @Permissions('inventory:write')
  @ApiOperation({ summary: 'Create a new assembly/kit plan', operationId: 'Inventory_CreateAssembly' })
  async createAssembly(@Req() req: any, @Body() body: any) {
    return this.assemblyUseCase.create(req.organization.id, req.user.memberId, body);
  }

  @Post('assemblies/:id/complete')
  @Permissions('inventory:write')
  @ApiOperation({ summary: 'Complete an assembly and update stock', operationId: 'Inventory_CompleteAssembly' })
  async completeAssembly(@Req() req: any, @Param('id') id: string, @Body('locationId') locationId: string) {
    return this.assemblyUseCase.complete(req.organization.id, req.user.memberId, id, locationId);
  }
  ...
  async requestAdjustment(@Req() req: any, @Body() body: any) {
    return this.requestAdjustmentUseCase.execute(req.organization.id, req.user.memberId, body);
  }

  @Patch('adjustments/:id/approve')
  @Permissions('inventory:manage')
  @ApiOperation({ summary: 'Approve a pending stock adjustment', operationId: 'Inventory_ApproveAdjustment' })
  async approveAdjustment(@Req() req: any, @Param('id') id: string) {
    return this.approveAdjustmentUseCase.execute(req.organization.id, req.user.memberId, id);
  }

  @Get('analytics/supplier-lead-time')
  @Permissions('inventory:read')
  @ApiOperation({ summary: 'Get supplier lead time analytics', operationId: 'Inventory_GetLeadTime' })
  ...
  @Get('analytics/waste')
  @Permissions('inventory:read')
  @ApiOperation({ summary: 'Get waste and shrinkage analysis', operationId: 'Inventory_GetWasteAnalysis' })
  async getWasteAnalysis(@Req() req: any, @Query('startDate') startDate?: string, @Query('endDate') endDate?: string) {
  ...
  async checkB2BAvailability(@Req() req: any, @Body() dto: CheckB2BAvailabilityDto) {
  ...
  @Permissions('inventory:write')
  @ApiOperation({ summary: 'Scan a batch QR code to receive and unpack it', operationId: 'Inventory_ScanUnpackBatch' })
  async scanUnpackBatch(@Req() req: any, @Body() body: { batchId: string }) {
    return this.scanUnpackBatchUseCase.execute(req.organization.id, req.user.memberId, body.batchId);
  }
  ...
  @ApiResponse({ status: 200, description: 'Availability report' })
  async quickInquiry(@Req() req: any, @Body() body: { businessAccountId: string; variantIds: string[] }) {
    return this.quickStockInquiryUseCase.execute(req.organization.id, body.businessAccountId, body.variantIds);
  }
}
