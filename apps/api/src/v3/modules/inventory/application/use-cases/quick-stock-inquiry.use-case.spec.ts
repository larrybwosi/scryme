...
describe('QuickStockInquiryUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [QuickStockInquiryUseCase, { provide: PrismaService, useValue: prisma }],
    }).compile();
  ...
  it('should return stock levels for a business account default location', async () => {
    ...
    prisma.client.businessAccount.findFirst.mockResolvedValue({ defaultLocationId: locId });
  ...
  it('should throw BadRequestException if business account has no default location', async () => {
    prisma.client.businessAccount.findFirst.mockResolvedValue({ defaultLocationId: null });
    await expect(useCase.execute('org-1', 'ba-1', [])).rejects.toThrow(BadRequestException);
  });
});
