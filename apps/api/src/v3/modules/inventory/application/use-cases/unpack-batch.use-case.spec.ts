...
describe('UnpackBatchUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new UnpackBatchUseCase(prisma as any, inventoryMovementService as any);
  });
  ...
  it('should unpack a batch successfully', async () => {
    ...
    expect(new Decimal(createCall.initialQuantity.toString()).toNumber()).toBe(48);
    expect(new Decimal(createCall.purchasePrice.toString()).toNumber()).toBe(10);
  ...
  it('should handle damages during unpacking', async () => {
    ...
    mockTx.stockBatch.create.mockImplementation(async params => {
      if (params.data.batchNumber.startsWith('DAMAGED')) return { id: 'damaged-batch' };
