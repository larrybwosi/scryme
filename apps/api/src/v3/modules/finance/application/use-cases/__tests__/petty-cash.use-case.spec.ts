...
describe('PettyCashUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [PettyCashUseCase, { provide: PrismaService, useValue: mockPrismaService }],
    }).compile();
  ...
  describe('createFund', () => {
    it('should create a fund and an initial transaction', async () => {
      ...
      mockPrismaClient.pettyCashFund.create.mockResolvedValue({ id: 'fund-1', ...dto });
  ...
  describe('topUpFund', () => {
    it('should increment fund amount and create a transaction', async () => {
      ...
      mockPrismaClient.pettyCashFund.findFirst.mockResolvedValue({ id: fundId, organizationId: orgId });
      mockPrismaClient.pettyCashFund.update.mockResolvedValue({ id: fundId, amount: 6000 });
    ...
    it('should throw NotFoundException if fund does not exist', async () => {
      ...
      await expect(useCase.topUpFund('org-1', 'invalid', { amount: 100 }, 'member-1'))
  ...
  describe('getFundById', () => {
    it('should return fund details', async () => {
      mockPrismaClient.pettyCashFund.findFirst.mockResolvedValue({ id: 'fund-1', name: 'Test Fund' });
    ...
    it('should throw if not found', async () => {
      mockPrismaClient.pettyCashFund.findFirst.mockResolvedValue(null);
      await expect(useCase.getFundById('org-1', 'fund-1')).rejects.toThrow(NotFoundException);
    });
  });
});
