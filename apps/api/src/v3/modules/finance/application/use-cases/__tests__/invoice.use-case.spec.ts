...
describe('InvoiceUseCase', () => {
  ...
  const mockPrisma = {
    client: {
      invoice: { create: vi.fn(), findMany: vi.fn(), findFirst: vi.fn(), findUnique: vi.fn(), update: vi.fn(), delete: vi.fn() },
  ...
  describe('Core Functionality', () => {
    ...
    it('should throw NotFoundException if invoice not found', async () => {
      ...
      await expect(useCase.getInvoiceById('org-1', 'inv-1')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('Partially Paid Invoices', () => {
    it('should create a partially paid invoice from a transaction', async () => {
      ...
      const mockOrder = {
        id: orderId, organizationId: orgId, subtotal: 100, finalTotal: 116, totalPaid: 50,
        items: [{ sku: 'SKU1', productName: 'P1', variantName: 'V1', quantity: 1, unitPrice: 100, lineTotal: 100 }],
        businessAccount: { name: 'Customer 1' }
      };
      mockPrisma.client.transaction.findFirst.mockResolvedValue(mockOrder);
      mockPrisma.client.invoice.create.mockResolvedValue({ id: 'inv-1', status: 'PARTIALLY_PAID', amountPaid: 50 });

      const result = await useCase.createInvoiceFromOrder(orgId, orderId);

      expect(result).toBeDefined();
      expect(mockPrisma.client.invoice.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should sync invoice status when fetched', async () => {
      ...
      const invId = 'inv-1';
      const mockInvoice = {
        id: invId, organizationId: orgId, status: 'UNPAID', amountPaid: 0,
        transaction: { id: 'order-1', finalTotal: 116, totalPaid: 116 }
      };
      mockPrisma.client.invoice.findFirst.mockResolvedValue(mockInvoice);
      mockPrisma.client.invoice.update.mockResolvedValue({ ...mockInvoice, status: 'PAID', amountPaid: 116 });

      const result = await useCase.getInvoiceById(orgId, invId);

      expect(mockPrisma.client.invoice.update).toHaveBeenCalledWith(expect.objectContaining({
        where: { id: invId },
        data: expect.objectContaining({
          status: 'PAID',
          amountPaid: 116
