...
export class UpdateOrderStatusUseCase {
  ...
  private async awardLoyaltyPoints(order: any) {
    ...
    try {
      const points = await this.loyaltyService.calculatePointsForTransaction(order.id);
      if (points > 0) {
  ...
  private async generateInvoice(order: any) {
    try {
      await this.invoiceUseCase.createInvoiceFromOrder(order.organizationId, order.id);
    } catch (error) {
      console.error(`Failed to auto-generate invoice for order ${order.id}:`, error);
    }
  }
}
