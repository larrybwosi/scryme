...
describe('RequestB2BQuoteUseCase', () => {
  ...
  it('should throw BadRequestException if location cannot be resolved', async () => {
    ...
    await expect(
      useCase.execute('org1', { items: [] }),
    ).rejects.toThrow(BadRequestException);
  });

  it('should throw BadRequestException if variant stock is insufficient', async () => {
    mockPrisma.client.inventoryLocation.findFirst.mockResolvedValue({ id: 'loc1' });
  ...
  it('should aggregate quantities for duplicate variantIds in request', async () => {
    ...
    vi.mocked(pricingResolver.resolveVariantPrice).mockResolvedValue({ unitPrice: 20 });
    ...
    expect(pricingResolver.resolveVariantPrice).toHaveBeenCalledWith(expect.objectContaining({
        quantity: 10
    }));
  });
});
