import { Controller, Get, Post, UseGuards, UseInterceptors, Req, Query, Param, Body } from '@nestjs/common';
...
export class OrderController {
  ...
  @ApiResponse({ status: 200, type: [OrderResponseDto], description: 'Paginated list of orders' })
  @ApiResponse({ status: 401, type: ApiErrorResponseDto, description: 'Unauthorized' })
  async getOrders(@Req() req: any, @Query() paginationQuery: PaginationQueryDto) {
  ...
  @ApiResponse({ status: 400, type: ApiErrorResponseDto, description: 'Invalid input or insufficient stock' })
