import { Controller, Get, Post, Delete, Body, UseGuards, Request, UseInterceptors } from '@nestjs/common';
...
import { FavoriteDto, FavoriteResponseDto } from '../../application/dto/favorite.dto';
...
export class FavoritesController {
  ...
  async addFavorite(@Request() req: any, @Body() dto: FavoriteDto) {
    ...
    return this.favoritesUseCase.addFavorite(req.organization.id, { ...dto, customerId });
  }
  ...
  async removeFavorite(@Request() req: any, @Body() dto: FavoriteDto) {
    const customerId = req.v3Context?.memberId;
    return this.favoritesUseCase.removeFavorite(req.organization.id, { ...dto, customerId });
  }
}
