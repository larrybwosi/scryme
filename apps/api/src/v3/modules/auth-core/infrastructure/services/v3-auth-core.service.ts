...
export class V3AuthCoreService {
  ...
  private async validateLoginMember(
    ...
    if (cardId) {
      ...
      // performing a cryptographically heavy bcrypt.compare on a valid dummy PIN hash if member or pinHash is missing/inactive.
      const dummyPinHash =
        "$2b$10$vI8tYnK6YKMH3O84S4eXQuKBLN3F3k4pXFmF0a.a2H88tM8vO6PzO";
      const pinHashToCompare =
        member && member.isActive && member.pinHash
          ? member.pinHash
          : dummyPinHash;
      const isPinValid = await bcrypt.compare(pin, pinHashToCompare);

      if (member && member.isActive && member.pinHash && isPinValid) {
