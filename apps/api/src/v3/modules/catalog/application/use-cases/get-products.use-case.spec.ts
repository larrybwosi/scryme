...
describe('GetProductsUseCase', () => {
  ...
  it('should return products for an organization', async () => {
    const orgId = 'org-1';
    const products = [new Product('1', 'Prod 1', 'Desc', orgId, 'cat-1', new Date(), new Date())];
    repository.findByOrganization.mockResolvedValue(products);

    const result = await useCase.execute(orgId, { limit: 10, offset: 0 });
    expect(result).toEqual(products);
    expect(repository.findByOrganization).toHaveBeenCalledWith(orgId, { limit: 10, offset: 0 });
  });
});
