...
describe("WindmillCallbackUseCase - Signature Verification", () => {
  ...
  it("should verify a valid signature", async () => {
    vi.spyOn(
      prismaService.client.windmillConfiguration,
      "findUnique",
    ).mockResolvedValue({
    ...
    await expect(
      useCase.verifySignature(mockOrganizationId, signature, mockPayload),
  ...
  it("should throw UnauthorizedException if signature is missing", async () => {
    ...
    await expect(
      useCase.verifySignature(mockOrganizationId, "", mockPayload),
    ).rejects.toThrow(UnauthorizedException);
  });
  ...
  it("should skip verification in development if secret is missing", async () => {
    ...
    await expect(
      useCase.verifySignature(mockOrganizationId, "any", mockPayload),
    ).resolves.not.toThrow();
  });
