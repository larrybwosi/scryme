...
export class WindmillCallbackController {
  ...
  async handleApprovalCallback(@Body() payload: ApprovalCallbackPayload) {
    try {
      ...
      throw new InternalServerErrorException('Failed to process approval callback');
    }
  }

  @Post('bakery/disposal')
  @ApiOperation({ summary: 'Handle bakery disposal callbacks from Windmill (V3)' })
  async handleBakeryDisposalCallback(@Body() payload: BakeryDisposalCallbackPayload) {
    try {
      ...
      throw new InternalServerErrorException('Failed to process bakery disposal callback');
    }
  }

  @Post('outcome')
  @ApiOperation({ summary: 'Handle generic outcome callbacks from Windmill (V3)' })
  async handleOutcomeCallback(@Body() payload: GenericOutcomePayload) {
    try {
      ...
      throw new InternalServerErrorException('Failed to process outcome callback');
    }
  }
}
