...
import { CreateCrmRelationshipDto, CreateCrmAssociationDto } from '../dto/crm-relationships.dto';

@Injectable()
export class CrmRelationshipService {
  constructor(private readonly prisma: PrismaService) {}

  async createRelationshipDefinition(organizationId: string, dto: CreateCrmRelationshipDto) {
    const existing = await this.prisma.client.crmRelationshipDefinition.findFirst({
      where: { organizationId, name: dto.name },
    });

    if (existing) {
      throw new ConflictException(`Relationship definition with name "${dto.name}" already exists`);
    }
  ...
  async createAssociation(organizationId: string, dto: CreateCrmAssociationDto) {
    ...
    const [source, target] = await Promise.all([
      this.prisma.client.crmRecord.findFirst({ where: { id: dto.sourceRecordId, organizationId } }),
      this.prisma.client.crmRecord.findFirst({ where: { id: dto.targetRecordId, organizationId } }),
    ]);
