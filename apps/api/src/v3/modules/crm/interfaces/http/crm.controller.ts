...
import { CreateCrmObjectDto, CreateCrmFieldDto } from '../../application/dto/crm-definitions.dto';
import { CreateCrmActivityDto } from '../../application/dto/crm-activity.dto';
import { CreateCrmRelationshipDto, CreateCrmAssociationDto } from '../../application/dto/crm-relationships.dto';
...
export class CrmController {
  ...
  @ApiResponse({ status: 200, type: CrmRecordResponseDto })
  async updateRecord(@Request() req: any, @Param('id') id: string, @Body() dto: UpdateCrmRecordDto) {
  ...
  async createNote(@Request() req: any, @Body() dto: CreateCrmNoteDto) {
    return this.noteService.createNote(req.organization.id, req.user.memberId, dto);
  }
  ...
  async createActivity(@Request() req: any, @Body() dto: CreateCrmActivityDto) {
    return this.activityService.createActivity(req.organization.id, req.user.memberId, dto);
  }

  @Get('records/:id/timeline')
  @ApiOperation({ summary: 'Get a unified timeline of notes and activities for a CRM record' })
  ...
  async createObject(@Request() req: any, @Body() dto: CreateCrmObjectDto) {
    return this.definitionService.createObjectDefinition(req.organization.id, dto);
  }
  ...
  async createField(@Request() req: any, @Param('objectId') objectId: string, @Body() dto: CreateCrmFieldDto) {
    return this.definitionService.createFieldDefinition(req.organization.id, objectId, dto);
  }
  ...
  async listFields(@Request() req: any, @Param('objectId') objectId: string) {
    return this.definitionService.getFieldsForObject(req.organization.id, objectId);
  }
  ...
  async createRelationship(@Request() req: any, @Body() dto: CreateCrmRelationshipDto) {
    return this.relationshipService.createRelationshipDefinition(req.organization.id, dto);
  }
  ...
  async createAssociation(@Request() req: any, @Body() dto: CreateCrmAssociationDto) {
  ...
  async listRecordAssociations(@Request() req: any, @Param('id') id: string) {
    return this.relationshipService.getAssociationsForRecord(req.organization.id, id);
  }
}
