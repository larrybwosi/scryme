...
describe('ProcessSaleUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: LoyaltyService, useValue: { calculatePointsForTransaction: vi.fn().mockResolvedValue(0) } },
        { provide: InvoiceUseCase, useValue: { createInvoiceFromOrder: vi.fn().mockResolvedValue({}) } },
      ],
    }).compile();
  ...
  it('should process a sale and update stock in parallel', async () => {
    const ctx = { organizationId: 'org_1', memberId: 'mem_1', locationId: 'loc_1' };
    const dto = {
      items: [
    ...
    prisma.client.productVariant.findMany.mockResolvedValue([
      { id: 'v1', retailPrice: 10, buyingPrice: 5, name: 'V1', sku: 'S1', product: { name: 'P1' } },
      { id: 'v2', retailPrice: 20, buyingPrice: 10, name: 'V2', sku: 'S2', product: { name: 'P2' } },
    ]);

    prisma.client.transaction.create.mockResolvedValue({ id: 't1', number: 'T1' });
  ...
  it('should throw error if variants are missing', async () => {
    ...
    await expect(useCase.execute(ctx, dto)).rejects.toThrow(BadRequestException);
  });
});
