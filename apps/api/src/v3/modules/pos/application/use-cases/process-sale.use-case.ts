...
export class ProcessSaleUseCase {
    ...
    return await this.prisma.client.$transaction(async (tx: any) => {
      ...
      const t = await tx.transaction.create({
        data: {
          ...
          notes: dto.notes,
          items: { create: items },
          loyaltyVouchers: dto.loyaltyVoucherCode ? { connect: { code: dto.loyaltyVoucherCode } } : undefined,
        },
  ...
  private async getV(tx: any, items: any[]) {
    ...
    const v = await tx.productVariant.findMany({
      where: { id: { in: ids } },
      select: { id: true, retailPrice: true, buyingPrice: true, name: true, sku: true, product: { select: { name: true } } },
    });
    if (v.length !== ids.length) throw new BadRequestException('Missing variants');
    return v;
  }
  ...
  private prepI(items: any[], variants: any[], organizationId: string) {
    return items.map(i => {
      ...
      return {
        variantId: v.id, productName: v.product.name, variantName: v.name, sku: v.sku || 'N/A',
        quantity: i.quantity, unitPrice: p, listPrice: p, unitCost: Number(v.buyingPrice || 0),
        subtotal: p * i.quantity, lineTotal: p * i.quantity, organizationId
      };
    });
  }
  ...
  private async getC(tx: any, orgId: string, phone?: string) {
    ...
    const c = await tx.customer.findFirst({ where: { organizationId: orgId, phone }, select: { id: true } });
    if (c) return c.id;
    const nc = await tx.customer.create({ data: { organizationId: orgId, phone, name: 'POS Customer' }, select: { id: true } });
    return nc.id;
  }

  private async vDisc(tx: any, code: string, cId: string, sub: number) {
    if (!code || !cId) return 0;
    const v = await tx.loyaltyVoucher.findUnique({ where: { code }, include: { reward: true } });
    this.valV(v, cId);
    const d = this.calcD(v.reward, sub);
    await tx.loyaltyVoucher.update({ where: { id: v.id }, data: { status: VoucherStatus.REDEEMED, redeemedAt: new Date() } });
    return d;
  }

  private valV(v: any, cId: string) {
    if (!v) throw new BadRequestException('No voucher');
    if (v.status !== VoucherStatus.ACTIVE) throw new BadRequestException('Inactive');
  ...
  private async stock(tx: any, orgId: string, locId: string, mId: string, items: any[], tId: string, tNo: string) {
    ...
    const stockUpdates = items.map(i =>
      tx.productVariantStock.update({
        where: { variantId_locationId: { variantId: i.variantId, locationId: locId } },
        data: {
  ...
  private done(orgId: string, tId: string, tNo: string, cId?: string) {
    this.loyaltyService.calculatePointsForTransaction(tId).then(p => {
      if (p > 0 && cId) this.loyaltyService.awardPoints(cId, p, orgId, `Points ${tNo}`, tId);
    }).catch(() => {});
