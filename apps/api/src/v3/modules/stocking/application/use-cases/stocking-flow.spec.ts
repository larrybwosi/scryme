...
vi.mock('@repo/db', async (importOriginal) => ({ ...(await importOriginal() as any),
  PurchaseStatus: {
    DRAFT: 'DRAFT',
    ORDERED: 'ORDERED',
...
describe('Stocking Flow Verification', () => {
  ...
  it('should verify the full flow: PO -> Receipt (QC) -> Transfer', async () => {
    ...
    mockTx.purchase.create.mockImplementation(({ data }: any) => ({
      ...
      totalAmount: 2000,
      currency: 'KES',
      items: data.items.create.map((item: any, index: number) => ({ id: `poi-${index}`, ...item })),
      member: { user: { name: 'Test User' } }
    }));

    const po = await purchaseOrderUseCase.create(mockOrgId, mockMemberId, createPoDto);
    ...
    const mockPoWithItems = {
      ...
      items: [
        { id: 'poi-0', variantId: 'v1', unitCost: 100, variant: { productId: 'p1' } },
        { id: 'poi-1', variantId: 'v2', unitCost: 200, variant: { productId: 'p2' } },
      ]
    };

    mockTx.purchase.findUnique.mockResolvedValue(mockPoWithItems);
    mockTx.stockReceipt.create.mockResolvedValue({ id: 'rec-1' });
    mockTx.stockBatch.create.mockImplementation(({ data }: any) => ({ id: `batch-${data.variantId}`, ...data }));
    ...
    const transfer = await stockTransferUseCase.create(mockOrgId, mockMemberId, createTransferDto);
    ...
    mockTx.productVariantStock.findUnique.mockResolvedValue({ availableStock: 10 });
    ...
    mockTx.stockTransfer.findUnique.mockResolvedValue({
        ...
        items: [{ id: 'tri-1', variantId: 'v1', requestedQuantity: 5, variant: { sku: 'SKU-V1' } }]
    });
    mockTx.stockBatch.findMany.mockResolvedValue([{ id: 'batch-v1', currentQuantity: 10 }]);
    ...
    mockTx.stockTransfer.findUnique.mockResolvedValue({
        ...
        items: [{ id: 'tri-1', variantId: 'v1', requestedQuantity: 5, unitCost: 100, variant: { productId: 'p1' } }]
    });

    await stockTransferUseCase.receive(mockOrgId, mockMemberId, 'tr-1', receiveTransferDto);
