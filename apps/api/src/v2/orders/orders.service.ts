...
export class OrdersService {
  ...
  async getOrders(ctx: V2ApiContext, query: any) {
    ...
    try {
      ...
      return { orders: shaped, pagination: { page, limit, total, totalPages: Math.ceil(total / limit) } };
    } catch (error) {
  ...
  async createOrder(ctx: V2ApiContext, body: any) {
    ...
    if (variants.length !== variantIds.length) {
      ...
      throw new NotFoundException({ message: 'One or more variants not found', missingVariantIds: missing });
    }
    ...
    if (insufficientStock.length > 0) {
      throw new ConflictException({ message: 'Insufficient inventory', insufficientStock });
    }
    ...
    const count = await this.prisma.client.transaction.count({ where: { organizationId: ctx.organizationId } });
    ...
    try {
      ...
      }).catch(err => console.error('[v2 orders POST] Failed to emit Windmill event:', err));
