...
export class UnitsController {
  ...
  async createOrganizationUnit(@v2Context() ctx: V2ApiContext, @Body() data: any) {
    return this.unitsService.createOrganizationUnit(ctx, data);
  }

  @Patch('organization/:id')
  @RequirePermission('product:manage:stock')
  async updateOrganizationUnit(@v2Context() ctx: V2ApiContext, @Param('id') id: string, @Body() data: any) {
    return this.unitsService.updateOrganizationUnit(ctx, id, data);
  }

  @Delete('organization/:id')
  @RequirePermission('product:manage:stock')
  async deleteOrganizationUnit(@v2Context() ctx: V2ApiContext, @Param('id') id: string) {
  ...
  async syncUnits(@v2Context() ctx: V2ApiContext, @Query('lastSync') lastSync?: string) {
