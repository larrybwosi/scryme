...
export class BakeryReportService {
  ...
  async generateAndSendReport(organizationId: string, days: number = 7) {
    this.logger.log(
      `Generating bakery production report for org ${organizationId} for the last ${days} days`,
    );
    ...
    const stats = await this.bakeryService.getProductionStats(
      organizationId,
      startDate,
      endDate,
    );
    ...
    if (stats.totalBatches === 0) {
      this.logger.log(
        `No production activity found for org ${organizationId} in the last ${days} days. Skipping report.`,
      );
      return;
    }

    const settings = await this.bakeryService.getSettings({
      organizationId,
    } as any);
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          e.response?.data?.code !== "ALREADY_EXISTS"
        ) {
          this.logger.warn(
            `Could not ensure Scryme channel ${channelSlug} exists: ${e.message}`,
          );
        }
      }
      ...
      this.logger.log(
        `Bakery production report sent to Scryme channel ${channelSlug} in workspace ${scrymeConfig.workspaceSlug}`,
      );
    } catch (error: any) {
      this.logger.error(
        `Failed to send bakery production report to Scryme: ${error.message}`,
      );
    }
  }
