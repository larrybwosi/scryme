...
export class ImageService {
    ...
    try {
      ...
      if (providerType === "sanity") {
        ...
        const response = await axios.get(originalUrl, {
          ...
          timeout: 5000,
          maxContentLength: 10 * 1024 * 1024, // 10MB limit to prevent DoS
        });
  ...
  private async getOriginalUrl(id: string): Promise<string> {
    ...
    if (providerType === "sanity") {
      ...
      // SSRF Prevention: Do not return arbitrary strings as URLs.
      throw new Error(`Invalid Sanity asset ID format: ${id}`);
    } else {
