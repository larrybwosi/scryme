## 2026-08-17 - [In-Memory Guest Cart Consolidation Before Parallelized Writes]
**Learning:** Parallelizing database writes (updates or creates) concurrently via `Promise.all` over dynamic guest cart inputs can introduce race conditions and lost updates if duplicate products or variants exist in the guest cart. Consolidating guest cart quantities in-memory by composite key (`productId_variantId` and `serviceId`) prior to generating write promises prevents duplicate database creates, guarantees accurate quantity aggregation, and enables safe $O(1)$ parallel write execution.
**Action:** Always consolidate duplicate array inputs in-memory before issuing concurrent `Promise.all` database update/create calls.

## 2026-08-15 - [Consolidated Variant Updates & Parallelized Item Writes in StockTransfer ship]
**Learning:** Performing sequential `productVariantStock.update` queries inside item shipment loops leads to database roundtrip latency and severe row-lock contention if multiple transfer items belong to the same product variant. Consolidating quantity adjustments per unique `variantId` in-memory first allows performing exactly 1 update query per unique variant concurrently via `Promise.all`. Furthermore, executing downstream `stockTransferItem.update` calls concurrently via `Promise.all` collapses transactional write latency from $O(N)$ down to a flat $O(1)$ concurrent roundtrip execution block.
**Action:** Always aggregate quantity adjustments by unique compound keys (like `variantId_locationId`) in-memory before issuing stock database updates, and parallelize non-dependent transactional record updates concurrently using `Promise.all`.

## 2026-08-13 - [Parallelized Safaricom M-Pesa STK Pushes in PosSaleService]
**Learning:** Sequential processing of high-latency external third-party payment API requests (such as `triggerStkPush` to Safaricom's M-Pesa gateway) inside loop structures introduces a severe execution bottleneck. Parallelizing them concurrently using `Promise.all` wrapped in localized `try/catch` handlers isolates network/API errors, prevents individual failures from disrupting other concurrent transactions, and dramatically accelerates response times for multiline or split payments from $O(N)$ down to a flat $O(1)$.
**Action:** Always wrap independent third-party API / high-latency network calls inside localized try/catch blocks and execute them concurrently via `Promise.all` to maintain lightning-fast response times.
