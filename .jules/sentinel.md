...
## 2026-06-14 - [Mitigated SSRF and DoS in Image Proxy]

**Vulnerability:** The `ImageService` blindly fetched any string provided as an image ID if it didn't match the Sanity ID pattern, enabling Server-Side Request Forgery (SSRF). Additionally, external fetches lacked timeouts and payload size limits, posing a Denial of Service (DoS) risk from slow or oversized responses.

**Learning:** Service-side proxies that fetch external resources must strictly validate inputs against expected patterns and never default to "raw fetch". Resource limits (timeout, max payload) are essential for defense-in-depth against DoS.

**Prevention:**

- Use strict regex or pattern matching for resource identifiers.
- Always implement timeouts and maximum content length for internal-to-external HTTP requests.
