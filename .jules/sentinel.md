...
## 2026-08-13 - Broken Object Level Authorization (BOLA/IDOR) in Config Lookups
...
## 2026-08-26 - Double-Spending Prevention & Null Expiration Check in Loyalty Engine
...
## 2026-08-31 - IDOR Vulnerability in Purchase Order Item Variant Association
**Vulnerability:** `PurchaseOrderUseCase.create` validated `supplierId` against the calling `organizationId`, but created purchase order items referencing user-supplied `variantId`s without verifying that the product variants belonged to the same organization context.
**Learning:** Checking top-level entity ownership (like supplier or location) is insufficient when array items (like product variants) are provided in the payload. Unvalidated array item foreign keys allow cross-tenant data association and inventory pollution.
**Prevention:** Always perform a bulk ownership count check (e.g. `tx.productVariant.count({ where: { id: { in: variantIds }, product: { organizationId } } })`) against unique input IDs inside write transactions before creating nested relations.
