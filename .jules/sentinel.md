...
## 2026-07-05 - Incomplete Authorization in WebSocket Event Handlers
**Vulnerability:** The V2 and V3 Realtime Gateways secured `join` and `publish` events but left `presence:get` and `history:get` entirely unprotected, allowing unauthorized data retrieval.
**Learning:** When securing WebSocket gateways, developers often focus on actions that "do" something (like joining a room or broadcasting) and overlook passive "get" actions. Since Socket.io handlers are individually decorated, each must be explicitly secured; global guards often don't apply or are bypassed in these specific event-driven contexts.
**Prevention:** Audit all `@SubscribeMessage` handlers in every gateway. Implement a mandatory `validateChannelAccess` check at the entry point of every handler that accepts a `channel` or record ID. Use automated unit tests to verify that every event handler returns an error when accessed with an unauthorized context.
