...
## 2026-07-30 - Insecure PRNG in Simulated API Keys and Job IDs
**Vulnerability:** In simulated API key generation and execution job identifiers inside `WorkflowsService` and `ScrymeService`, predictable math-based random functions (`Math.random()`) were utilized, leading to predictable API keys and job ID sequences.
**Learning:** Even simulated keys, development placeholders, or generic execution identifiers must be guarded against predictability. Attackers exploiting predictability in mock credentials or job tracking identifiers can brute-force, guess, or bypass authentication/validation mechanisms.
**Prevention:** Always utilize Node's built-in `crypto` library and employ cryptographically secure pseudo-random number generators (CSPRNG), such as `crypto.randomBytes()`, to generate secure and unpredictable alphanumeric or hex strings for simulated keys, identifiers, or tokens.
