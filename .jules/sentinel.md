...
## 2026-08-10 - Member cardId Timing Attack and PIN Enumeration in V3 check-in login
**Vulnerability:** In `V3AuthCoreService.validateLoginMember`, providing a `cardId` would check PIN credentials via `bcrypt.compare` only if a member was found, active, and possessed a hashed PIN. If any condition was unmet, the check returned immediately without running bcrypt, resulting in a timing side-channel that allowed cardId/member enumeration.
**Learning:** Login or check-in verification endpoints must ensure identical computational and timing footprint for both valid and invalid queries. Returning early before computing heavy hashes (like bcrypt) leaks structurally significant status differences to attackers.
**Prevention:** Always perform a dummy `bcrypt.compare` against a pre-calculated placeholder PIN hash (with identical work factor) if the queried entity, state, or password hash is not present, ensuring completely consistent response timing.
