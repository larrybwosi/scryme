...
## 2026-07-27 - Self-Banning Vulnerability in System Admin Controller
**Vulnerability:** The global V3 `AdminController.banUser` endpoint allowed authenticated SUPER_ADMINs to initiate a ban against their own user ID, which would disable their user account, suspend all their associated organization memberships, and effectively lock them out of the system without warning.
**Learning:** Critical administrative workflows must enforce self-protection constraints. Standard role-based access controllers (RBAC) ensure that only authorized actors can run an action, but they do not validate if the action operates destructively on the actor themselves. This leaves the application vulnerable to self-banning or self-disabling scenarios.
**Prevention:** Always resolve the caller's identity (e.g. `request.user.id`) from the security guard layer and intercept self-directed mutations (such as banning or deletion) directly in the controller or service layer before invoking the state transition.
