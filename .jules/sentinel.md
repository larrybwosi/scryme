...
## 2026-08-12 - Critical Account Takeover / Credential Overwrite in Customer Registration
**Vulnerability:** The customer registration use case was executing a blind `upsert` on the global `User` table when setting passwords. This allowed unauthenticated registration requests to overwrite the password of any existing user (including admins, developers, or other customers) with the same email, leading to full Account Takeover (ATO).
**Learning:** Multi-tenant checks inside tenant-scoped controllers do not protect global tables from cross-tenant/cross-user modifications. Performing blind `upsert` queries on columns with global unique constraints (like `email`) without validating if they are already populated allows unauthorized actors to overwrite sensitive credentials and hijack existing accounts.
**Prevention:** Always check if a global record exists before modifying or setting sensitive fields like `password`. If a `User` already exists and has an active password, throw a validation error (e.g. `BadRequestException`) to block unauthorized credential changes, and ensure password changes are exclusively restricted to authenticated/proven flows.
