...
## 2026-07-01 - [Mass Assignment and Cross-Tenant Member Hijacking in Bakery]
**Vulnerability:** `BakeryService.addBaker` and `updateBaker` used the `...data` spread pattern, allowing mass assignment of internal fields like `bakerySettingsId`. Additionally, `addBaker` did not verify that the `memberId` being added belonged to the authenticated organization, allowing users to link members from other organizations to their own bakery.
**Learning:** When a model (like `BakeryBaker`) lacks a direct `organizationId` and is instead linked through a configuration model (`BakerySettings`), developers often forget to verify the organization boundary for related input IDs (like `memberId`). Spreading untrusted request bodies into Prisma `create`/`update` calls is a high-risk pattern for mass assignment.
**Prevention:** Always use explicit property destructuring (whitelisting) for input data. For any foreign key provided in a request body (e.g., `memberId`), perform an explicit ownership check (e.g., `db.member.findFirst({ where: { id, organizationId } })`) before using it in a mutation.

## 2026-07-03 - [Global Guard Default-Allow Vulnerability]
**Vulnerability:** Discovered that several V2 API controllers (e.g., `CustomersController`, `MpesaController`) were missing authorization decorators. The global `AuthorizationGuard` defaults to allowing access if no `@Permissions` or `@RequirePermission` decorators are present, meaning any authenticated entity could access sensitive data.
**Learning:** Security-by-omission is a major risk in this architecture. Unlike "Default Deny" systems, this codebase requires explicit opt-in for RBAC on every endpoint. Authenticated users without specific roles can bypass authorization if the developer forgets a single decorator.
**Prevention:** Always apply `@RequirePermission` or `@AllowPublic` to every controller method. Audit all V2 controllers for missing decorators. Consider refactoring `AuthorizationGuard` to "Default Deny" to fail securely, although this would require a full audit of all existing endpoints.

## 2026-07-05 - [IDOR and Mass Assignment in Shared Services]
**Vulnerability:** The `SupplierService` (in `@repo/suppliers`) was vulnerable to IDOR because its mutation methods (`createQualityIncident`, `initiateRecall`, `createSupplierDocument`) were not verifying the ownership of related resource IDs (e.g., `supplierId`, `batchId`, `stockBatchId`) against the authenticated `organizationId`. It was also vulnerable to mass assignment by spreading untrusted `data` objects into Prisma calls.
**Learning:** Shared workspace packages often lack the robust security middleware found in the main API controllers. When services are decoupled from the request lifecycle, developers must manually re-verify all organization boundaries for every input ID.
**Prevention:** Always perform explicit ownership lookups (`findFirst({ where: { id, organizationId } })`) for all foreign keys provided in request bodies. Never use the spread operator (`...data`) for untrusted input; use explicit property destructuring (whitelisting) to ensure only intended fields are persisted.

## 2026-07-06 - [IDOR in Inventory Integrity and Assembly Completion]
...
## 2026-07-12 - [IDOR and Mass Assignment in Inventory Mutation Methods]
**Vulnerability:** Found that `InventoryService.createInventoryItem`, `updateInventoryItem`, and `createInventoryMovement` were using the spread operator (`...data`), allowing mass assignment of sensitive stock fields. Additionally, they were not verifying that input IDs (`locationId`, `variantId`, `inventoryId`) belonged to the authenticated `organizationId`.
**Learning:** Services that handle core resource creation often trust client-provided IDs if the main entity is scoped, but they fail to verify the "leaf" nodes of the relationship (like a location or a specific variant). Using spread for mutations is a persistent anti-pattern that bypasses security controls.
**Prevention:** Always use explicit field destructuring to whitelist allowed properties for creation and updates. For every foreign key or resource ID received from the client, perform an ownership check using `findFirstOrThrow` scoped by `organizationId` before using it in a database operation.

## 2026-07-15 - [Insecure Direct Object Reference (IDOR) in Loyalty Module]
**Vulnerability:** The `LoyaltyService` was vulnerable to IDOR because its methods (`awardPoints`, `redeemPointsForVoucher`, `getCustomerStatus`, `validateVoucher`) were retrieving or updating records (Customer, LoyaltyReward, LoyaltyVoucher) by ID without verifying their `organizationId`.
**Learning:** For models that lack a compound `[id, organizationId]` unique index, Prisma's `findUnique` cannot be used to enforce tenant isolation. In these cases, `findFirst` must be used with both the record ID and the `organizationId` in the `where` clause. Even in methods like `awardPoints` that use an `update` operation, the existence and ownership of the record must be verified first if the `update` doesn't include the tenant ID in its filter.
**Prevention:** Always use `findFirst` or `findFirstOrThrow` with `organizationId` scoping for multi-tenant record retrieval. For mutations on models without compound unique indexes, perform an explicit ownership check before the update/delete, or use `updateMany`/`deleteMany` with a composite filter.
