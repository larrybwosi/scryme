...
## 2025-07-01 - [Inventory Audit Modal UX Modernization]

**Learning:** Mixing native HTML elements (like `<select>`) and browser APIs (like `alert()`) with themed UI components creates a fragmented user experience and bypasses the design system's accessibility and styling benefits. Additionally, a missing reset of the submission state on validation failure can trap the UI in a "loading" state.
**Action:** Always use themed `Select` and `toast` for form controls and feedback. Ensure `isSubmitting` is reset to `false` if early returns occur during validation. Mark required fields with `<span className="text-red-500">*</span>` and include a `Loader2` spinner in the submit button.
