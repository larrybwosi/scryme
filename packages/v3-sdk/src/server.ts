...
> {
  ...
  constructor(config: ServerSDKConfig) {
    ...
    this.cart = {
      get: async <T = TCartResponse>(params?: CartControllerGetCartParams): Promise<AxiosResponse<T>> => {
    ...
    this.customer = {
      ...
      updateProfile: async <T = TUser>(customerId: string, dto: UpdateCustomerDto): Promise<AxiosResponse<T>> => {
...
export function createServerSDK<
  ...
  TUser = CustomerResponseDto,
  TSession = CustomerSessionDto
>(config: Partial<ServerSDKConfig> = {}): ScrymeServerSDK<TProduct, TService, TCartItem, TCartResponse, TUser, TSession> {
  const finalConfig = {
