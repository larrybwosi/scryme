...
const getA4Styles = (primaryColor = '#2563eb') => StyleSheet.create({
  page: {
    padding: 40,
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          <View>
            ...
            <Text style={{ fontSize: 9, color: '#64748b' }}>{data.branding?.companyAddress}</Text>
          </View>
          <View>
            ...
            <Text style={styles.docMeta}>Date: {format(new Date(data.date), 'MMM dd, yyyy')}</Text>
          </View>
        </View>
        ...
        <View style={styles.grid}>
          <View style={styles.addressBlock}>
            ...
            <Text style={{ fontWeight: 'bold', fontSize: 11 }}>{data.customer.name}</Text>
            {data.customer.email && <Text style={{ fontSize: 9 }}>{data.customer.email}</Text>}
            {data.customer.phone && <Text style={{ fontSize: 9 }}>{data.customer.phone}</Text>}
          </View>
          <View style={styles.addressBlock}>
            <Text style={styles.sectionTitle}>Shipping Address</Text>
            <Text style={{ fontSize: 10 }}>{formatAddress(data.shippingAddress)}</Text>
          </View>
        </View>
        ...
        <View style={styles.table}>
          ...
          {data.items.map((item, index) => (
            <View key={index} style={styles.tableRow}>
              <Text style={[styles.colSku, { fontSize: 9 }]}>{item.sku || '-'}</Text>
        ...
        <View style={styles.signatureArea}>
          ...
          <View style={styles.signBox}>
            <View style={styles.signLine} />
            <Text style={styles.signLabel}>Received By (Customer Signature)</Text>
          </View>
        </View>

        <View style={styles.footer}>
          <Text style={{ fontSize: 8, color: '#94a3b8', textAlign: 'center' }}>
            Thank you for your business. Please retain this delivery note for your records.
          </Text>
        </View>
      </Page>
    </Document>
