...
const getThermalStyles = (primaryColor = "#000000") =>
  StyleSheet.create({
    page: {
      padding: 10,
      width: 226,
    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.table}>
        <View style={styles.tableHeader}>
          ...
          <Text style={[styles.tableHeaderCol, styles.colOrdered]}>
            Ordered
          </Text>
        ...
        {data.items.map((item, index) => (
          <View key={index} style={[styles.tableRow, { alignItems: "center" }]}>
            ...
            <Text style={[styles.colPacked]}>
              {item.quantityPacked ?? "___"}
            </Text>
          </View>
        ))}
      </View>
...
export const PackingListDocument = ({
  data,
  format = "A4",
}: {
  ...
  return (
    <Document>
      {format === "THERMAL" ? (
        <ThermalPackingList data={data} />
      ) : (
        <A4PackingList data={data} />
      )}
    </Document>
  );
};
