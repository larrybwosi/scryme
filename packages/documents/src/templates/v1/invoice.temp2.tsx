...
const InvoiceHeader = ({ invoice }: { invoice: InvoiceData }) => {
  ...
  return (
    <View style={styles.headerView}>
      <View>
        {branding?.logoUrl && (
          <Image style={styles.companyLogo} src={branding.logoUrl} />
        )}
        <Text style={styles.h3}>{branding?.companyName || "Company Name"}</Text>
        <Text style={styles.address}>{branding?.companyAddress || ""}</Text>
      </View>
    ...
    {invoice.items.map((item, index) => (
      <View key={index} style={styles.tableRow}>
        ...
        <Text style={[styles.tableCell, styles.colAmount]}>
          {formatCurrency(
            item.totalPrice || item.amount || item.totalPrice || 0,
            invoice.currencySymbol,
          )}
        </Text>
      </View>
...
const InvoiceFooter = ({
  ...
  <View style={styles.footer} fixed>
    ...
    {(qrCode || invoice.qrCode) && (
      <Image style={styles.footerQRCode} src={qrCode || invoice.qrCode} />
    )}
  </View>
);
