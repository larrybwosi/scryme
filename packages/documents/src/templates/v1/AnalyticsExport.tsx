...
export const AnalyticsExport = ({ data }: { data: TransactionAnalyticsExportData }) => {
  ...
  <Document>
    <Page style={styles.page} size="A4" orientation="landscape">
      ...
      <View style={styles.filters}>
        ...
        {data.activeFiltersText && <Text>Filters: {data.activeFiltersText}</Text>}
      </View>
      ...
      <View style={styles.table}>
        ...
        {data.transactions.map((t, i) => (
          <View key={i} style={styles.tableRow}>
            ...
            <Text style={[styles.col, styles.colCustomer]}>{t.customerName}</Text>
            <Text style={[styles.col, styles.colItems]}>
              {t.items.map(item => `${item.productName} (x${item.quantity})`).join(', ')}
            </Text>
            <Text style={[styles.col, styles.colTotal]}>{t.total.toFixed(2)}</Text>
          </View>
        ))}
      </View>
    </Page>
