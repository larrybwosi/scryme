...
export const BusinessInvoicePDF: React.FC<{ data: InvoiceData }> = ({
  ...
  const grandTotal =
    invoiceData.total || invoiceData.grandTotal || subTotal + (taxAmount ?? 0);

  // Installment Logic
  const isInstallment = invoiceData.installmentDetails?.isInstallment;
  const amountPaid =
    invoiceData.amountPaid ||
    invoiceData.installmentDetails?.totalAmountPaidSoFar ||
    0;
  const balanceDue =
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          <View>
            ...
            <Text style={styles.invoiceInfo}>
              <Text style={styles.invoiceLabel}>
                Date Of Issue: {String(invoiceData.date) as any}
              </Text>
            </Text>
            <Text style={styles.invoiceInfo}>
              <Text style={styles.invoiceLabel}>
        ...
        <View style={styles.billingSection}>
          ...
          <View style={[styles.billColumn, { alignItems: "flex-end" }]}>
            <Text style={[styles.billTitle, { textAlign: "right" }]}>
              From:
            </Text>
            <Text style={[styles.billText, { textAlign: "right" }]}>
          ...
          <View style={styles.totalsBox}>
            <View style={styles.totalRow}>
              <Text style={styles.totalLabel}>
                {isInclusive ? "Total (Excl. Tax)" : "Sub Total"}
              </Text>
              <Text style={styles.totalValue}>
                {formatPrice(subTotal, code || "USD", locale || "en-US")}
              </Text>
            </View>
            <View style={styles.totalRow}>
              <Text style={styles.totalLabel}>
              ...
              <Text style={styles.totalValue}>
                {formatPrice(taxAmount, code || "USD", locale || "en-US")}
              </Text>
            </View>
            ...
            {isInstallment ? (
              <>
                ...
                <View style={styles.totalRow}>
                  ...
                  <Text style={styles.totalValue}>
                    ({formatPrice(amountPaid, code || "USD", locale || "en-US")}
                    )
                  </Text>
                </View>
                <View style={[styles.totalRow, styles.balanceDueRow]}>
                  ...
                  <Text style={styles.balanceDueValue}>
                    {formatPrice(balanceDue, code || "USD", locale || "en-US")}
                  </Text>
                </View>
              </>
            ) : (
