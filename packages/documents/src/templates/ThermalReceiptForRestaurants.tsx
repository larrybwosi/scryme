...
const getStyles = (primaryColor = '#000') => StyleSheet.create({
  page: {
    fontFamily: 'Roboto',
    fontSize: 9,
    padding: 10,
...
export function ThermalReceiptPDF({ order, qrCode }: ThermalReceiptPDFProps) {
  const branding = order.branding || { primaryColor: order.organization.primaryColor };
    ...
    <Document>
      <Page size={{ width: 226, height: 'auto' }} style={styles.page}>
        <View style={styles.header}>
          ...
          {order.organization.address && <Text style={styles.headerText}>{order.organization.address}</Text>}
          {order.organization.phone && <Text style={styles.headerText}>Phone: {order.organization.phone}</Text>}
        </View>

        <Text style={styles.title}>Order: {order.orderNumber}</Text>
        {order.tableNumber && <Text style={styles.title}>Table: {order.tableNumber}</Text>}
        <Text style={styles.headerText}>Date: {new Date(order.placedAt).toLocaleString('en-KE')}</Text>
        <Text style={styles.headerText}>Served by: {order.member?.name || 'N/A'}</Text>
