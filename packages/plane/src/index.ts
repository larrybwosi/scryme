...
export class PlaneApiClient {
  ...
  constructor(
    private readonly baseUrl: string = process.env.PLANE_API_URL ||
      "https://app.plane.so",
  ...
  async createProject(
    ...
    const response = await this.client.post<PlaneProject>(
  ...
  async getProject(
    ...
    const response = await this.client.get<PlaneProject>(
