...
const CrmPermissionsContext = createContext<CrmPermissionsContextValue | null>(
  null,
);
...
export const useCrmPermissions = () => {
  ...
  if (!context) {
    throw new Error(
      "useCrmPermissions must be used within CrmPermissionsProvider",
    );
  }
  return context;
};
...
export const CrmPermissionsProvider: React.FC<CrmPermissionsProviderProps> = ({
  ...
  const value = useMemo<CrmPermissionsContextValue>(() => {
    const isAdmin =
      permissions.includes("*") || permissions.includes("crm:manage");
  ...
  return (
    <CrmPermissionsContext.Provider value={value}>
      {children}
...
export const CrmAccessLevel: React.FC<CrmAccessLevelProps> = ({
  className,
}) => {
  ...
  const level = useMemo(() => {
    if (isAdmin)
      return { label: "Full Access", color: "bg-emerald-500", icon: Shield };
    if (hasPermission(CRM_PERMISSIONS.RECORD_MANAGE)) {
      return { label: "Manager", color: "bg-blue-500", icon: Shield };
    }
    if (
      hasPermission([
        CRM_PERMISSIONS.RECORD_VIEW,
        CRM_PERMISSIONS.RECORD_VIEW_OWN,
      ])
    ) {
