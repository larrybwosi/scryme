...
export const SupplierDetailView: React.FC<SupplierDetailViewProps> = ({
  ...
  const { data: deliveries, isLoading: isLoadingDeliveries } =
    useGetSupplierDeliveries(supplier.id);
  const {
    data: products,
    isLoading: isLoadingProducts,
    refetch: refetchProducts,
  } = useListSupplierProducts(supplier.id);
  const { data: analytics, isLoading: isLoadingAnalytics } =
    useGetSupplierAnalytics(supplier.id);
  ...
  return (
    <div className="flex flex-col gap-6 p-6 bg-slate-50/50 min-h-screen">
              ...
              <Badge
                className={
                  supplier.status === "active"
                    ...
                    : (supplier.status as string) === "on_hold"
        ...
        <div className="lg:col-span-3">
          <Tabs defaultValue="products" className="w-full">
            <TabsList className="w-full justify-start border-b rounded-none h-12 bg-transparent p-0 mb-6">
              <TabsTrigger
                value="products"
                className="data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none h-12 px-6"
              >
                Products
              </TabsTrigger>
              <TabsTrigger
              ...
              <Card className="shadow-sm border-slate-200">
                <CardHeader className="flex flex-row items-center justify-between pb-4">
                  <div>
                    ...
                    <CardDescription>
                      All products currently sourced from {supplier.name}
                    </CardDescription>
                  </div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-y bg-slate-50/50">
                          ...
                          <th className="h-10 px-4 text-left font-semibold text-slate-600">
                            Product Specification
                          </th>
                          <th className="h-10 px-4 text-left font-semibold text-slate-600">
                            SKU / Ref
                          </th>
                          <th className="h-10 px-4 text-right font-semibold text-slate-600">
                            Current Cost
                          </th>
                          <th className="h-10 px-4 text-center font-semibold text-slate-600">
                            Status
                          </th>
                          <th className="h-10 px-4 text-right font-semibold text-slate-600">
                            Actions
                            ...
                            >
                              <td className="px-4 py-3">
                                <Checkbox
                                  ...
                                  onCheckedChange={() =>
                                    toggleProductSelection(p.productId)
                                  }
                                />
                              </td>
                              ...
                              <td className="px-4 py-3 text-center">
                                <Badge
                                  variant={
                                    p.isPreferred ? "default" : "secondary"
                                  }
                                  className={p.isPreferred ? "bg-blue-600" : ""}
                                >
      ...
      <UpdatePriceDialog
        ...
        onOpenChange={(open) =>
          setUpdatePriceData((prev) => ({ ...prev, open }))
        }
        supplierId={supplier.id}
      ...
      <PriceHistoryDialog
        ...
        onOpenChange={(open) =>
          setPriceHistoryData((prev) => ({ ...prev, open }))
        }
        supplierId={supplier.id}
