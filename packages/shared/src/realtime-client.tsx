...
export const RealtimeProvider = ({ children }: { children: ReactNode }) => {
  ...
  const subscribe = useCallback((channelName: string, event: string, callback: (data: any) => void, options?: { rewind?: number }) => {
    if (provider === 'socketio' && socket) {
      socket.emit('join', { channel: channelName, options });

      const presenceHandler = (data: { channel: string, members: PresenceMember[] }) => {
        if (data.channel === channelName) {
          setPresenceMembers(prev => ({ ...prev, [channelName]: data.members }));
        }
      };
      ...
      if (options?.rewind) {
        ...
        const rewindChannel = ably.channels.get(`[?rewind=${options.rewind}]${channelName}`);
  ...
  const enterPresence = useCallback(async (channelName: string, metadata?: any) => {
    if (provider === 'socketio' && socket) {
