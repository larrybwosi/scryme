...
export class StorageService implements StorageProvider {
  ...
  async upload(file: Buffer, filename: string, contentType: string, options?: { uploadAsFile?: boolean; encrypt?: boolean }) {
  ...
  async startMultipartUpload(filename: string, contentType: string) {
    ...
    throw new Error('Multipart uploads are not supported by the current storage provider');
  }

  async uploadPart(filename: string, uploadId: string, partNumber: number, body: Buffer) {
  ...
  async completeMultipartUpload(filename: string, uploadId: string, parts: { ETag: string; PartNumber: number }[]) {
