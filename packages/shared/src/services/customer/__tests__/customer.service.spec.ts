...
describe("CustomerService Shared", () => {
  ...
  describe("getCustomerById", () => {
    it("should aggregate totals and limit related records", async () => {
      ...
      (prisma.customer.findFirst as jest.Mock).mockResolvedValue(mockCustomer as any);
      (prisma.invoice.aggregate as jest.Mock).mockResolvedValueOnce({ _sum: { grandTotal: 1000 } } as any); // total
      (prisma.invoice.aggregate as jest.Mock).mockResolvedValueOnce({ _sum: { grandTotal: 400 } } as any);  // paid
    ...
    it("should handle null aggregates", async () => {
        (prisma.customer.findFirst as jest.Mock).mockResolvedValue({ id: '1', name: 'N' } as any);
        (prisma.invoice.aggregate as jest.Mock).mockResolvedValue({ _sum: { grandTotal: null } } as any);
