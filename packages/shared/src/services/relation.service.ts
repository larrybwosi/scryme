...
export class RelationService {
  ...
  async associate(
    ...
    if (!source || !target)
      throw new Error("Source or target record not found");
    if (
      source.organizationId !== rel.organizationId ||
      target.organizationId !== rel.organizationId
    ) {
      throw new Error(
        "Tenant isolation violation: Records must belong to the relationship organization",
      );
    }
