export function getOpenLoyaltyConfig() {
  ...
  if (!apiKey)
    throw new Error("OPEN_LOYALTY_API_KEY environment variable is not set");
...
export const OpenLoyaltyClient = {
  ...
  async registerCustomer(customerId: string, email: string, phone?: string) {
    ...
    if (!res.ok)
      throw new Error(
        `Failed to register customer in Open Loyalty: ${await res.text()}`,
      );
    return res.json();
  },
  ...
  async submitTransaction(
    ...
    if (!res.ok)
      throw new Error(`Failed to submit transaction: ${await res.text()}`);
    return res.json();
  },
  ...
  async addPoints(customerId: string, points: number, reason: string) {
    ...
    const res = await fetch(
      `${url}/api/${storeCode}/customer/${customerId}/points/add`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${apiKey}`,
  ...
  async deductPoints(customerId: string, points: number, reason: string) {
    const { url, apiKey, storeCode } = getOpenLoyaltyConfig();
    const res = await fetch(
      `${url}/api/${storeCode}/customer/${customerId}/points/spend`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${apiKey}`,
    ...
    if (!res.ok)
      throw new Error(`Failed to deduct points: ${await res.text()}`);
    return res.json();
  },
  ...
  async getCustomerTransactions(customerId: string) {
    const { url, apiKey, storeCode } = getOpenLoyaltyConfig();
    const res = await fetch(
      `${url}/api/${storeCode}/customer/${customerId}/transaction`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${apiKey}`,
