...
const suppliers = {
  ...
  products: {
    ...
    delete: async (supplierId: string, productId: string): Promise<any> =>
      axios
        ...
        .then(res => res.data),
    updatePrice: async (supplierId: string, productId: string, data: { costPrice: number }): Promise<any> =>
      axios.patch(`/api/suppliers/${supplierId}/products/${productId}/price`, data).then(res => res.data),
    bulkUpdate: async (supplierId: string, data: any): Promise<any> =>
      axios.post(`/api/suppliers/${supplierId}/products/bulk`, data).then(res => res.data),
  },
  ...
  priceHistory: async (supplierId: string, variantId?: string): Promise<PriceHistoryEntry[]> =>
    axios.get(`/api/suppliers/${supplierId}/price-history`, { params: { variantId } }).then(res => res.data),
};
...
export const useUpdateSupplier = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ supplierId, data }: { supplierId: string; data: any }) => await suppliers.update(supplierId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['suppliers'] });
    },
  });
...
export const useDeleteSupplierProduct = (supplierId: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (productId: string) => await suppliers.products.delete(supplierId, productId),
    onSuccess: () => {
...
export const useAddProductSupplier = (supplierId: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (data: any) => await suppliers.products.create(supplierId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['supplier-products', supplierId] });
    },
  });
...
export const useUpdateProductPrice = (supplierId: string) => {
  ...
  return useMutation({
    mutationFn: async ({ productId, costPrice }: { productId: string; costPrice: number }) =>
        await suppliers.products.updatePrice(supplierId, productId, { costPrice }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['supplier-products', supplierId] });
      queryClient.invalidateQueries({ queryKey: ['supplier-price-history', supplierId] });
    },
  });
};
...
export const useBulkUpdateSupplierProducts = (supplierId: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (data: any) => await suppliers.products.bulkUpdate(supplierId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['supplier-products', supplierId] });
    },
  });
...
export const useGetSupplierDeliveries = (supplierId: string) => {
  return useQuery({
    ...
    queryFn: async () => {
        const response = await axios.get(`/api/suppliers/${supplierId}/deliveries`);
...
export const useGetSupplierPriceHistory = (supplierId: string, variantId?: string) => {
  return useQuery({
