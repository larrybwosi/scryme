...
export class NavariService {
  ...
  private async validateAccess(organizationId: string, options: { taxOnly?: boolean } = {}) {
    if (await this.isGloballyDisabled()) {
      throw new Error('Navari service is temporarily disabled by system administrator');
    }
    ...
    if (options.taxOnly && settings?.country !== 'Kenya') {
      throw new Error('Navari tax integration is only available for Kenyan organizations');
    }
  ...
  async setupOrganization(organizationId: string, credentials: { apiKey: string; apiSecret: string }) {
    ...
    return await db.organizationIntegration.upsert({
      ...
      update: {
        credentials: { apiKey: credentials.apiKey, apiSecret: credentials.apiSecret } as any,
        isActive: true,
      },
      create: {
        organizationId,
  ...
  private async getAuthToken(organizationId: string) {
    ...
    if (!integration || !integration.credentials) {
      throw new Error('Navari integration not configured for this organization');
    }
  ...
  async validateKRAPin(organizationId: string, kraPin: string) {
    const { apiKey, apiSecret } = await this.validateAccess(organizationId, { taxOnly: true });

    try {
      const response = await this.client.post('/kra/validate-pin', {
        kraPin,
      }, {
        headers: {
          'X-API-KEY': apiKey,
      ...
      return {
        valid: isValidFormat,
        message: isValidFormat ? 'KRA PIN format is valid' : 'Invalid KRA PIN format',
      };
    }
  }
  ...
  async generateETRInvoice(organizationId: string, invoiceData: any) {
    ...
    try {
      ...
      const navariError = new Error(`Failed to generate ETR invoice: ${error.message}`);
  ...
  async submitPurchaseToKRA(organizationId: string, purchaseData: any) {
    ...
    try {
      const response = await this.client.post('/kra/purchase/submit', purchaseData, {
        headers: {
          'X-API-KEY': apiKey,
          'X-API-SECRET': apiSecret,
        },
      ...
      console.error('[Navari Service] Purchase submission failed:', error.message);
      throw new Error(`Failed to submit purchase to KRA: ${error.message}`);
    }
  }

  async getVATReturn(organizationId: string, params: { startDate: string; endDate: string }) {
    const { apiKey, apiSecret } = await this.validateAccess(organizationId, { taxOnly: true });
    ...
    try {
      ...
      console.error('[Navari Service] VAT return report failed:', error.message);
      throw error;
    }
  }
  ...
  async fileVATReturn(organizationId: string, returnData: any) {
    ...
    try {
      const response = await this.client.post('/kra/filing/vat-return', returnData, {
        headers: {
          'X-API-KEY': apiKey,
          'X-API-SECRET': apiSecret,
        },
  ...
  async verifyMpesaTransaction(organizationId: string, transactionId: string) {
    ...
    try {
      const response = await this.client.get(`/mpesa/transaction/${transactionId}`, {
        headers: {
          'X-API-KEY': apiKey,
          'X-API-SECRET': apiSecret,
        },
      ...
      console.error('[Navari Service] Transaction verification failed:', error.message);
      throw error;
    }
  }
}
