...
describe("Provisioning Logic", () => {
  ...
  describe("createDeviceSetupToken", () => {
    it("should create a setup token with correct data", async () => {
      ...
      (mockPrisma.deviceSetupToken.create as any).mockResolvedValue({
        id: "token-1",
        ...data,
      });
  ...
  describe("provisionDeviceV2", () => {
    it("should successfully redeem a valid token", async () => {
      ...
      (mockPrisma.deviceSetupToken.findFirst as any).mockResolvedValue(
        mockSetupToken,
      );
      (mockPrisma.apiKey.create as any).mockResolvedValue({ id: "key-1" });
      (mockPrisma.deviceRegistry.create as any).mockResolvedValue({
        id: "reg-1",
      });
      ...
      expect(mockPrisma.deviceSetupToken.update).toHaveBeenCalledWith(
