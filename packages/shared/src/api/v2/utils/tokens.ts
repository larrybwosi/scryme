...
export async function generateTransactionQR(
  transactionId: string,
  organizationId: string,
): Promise<string> {
...
export function generateDocumentToken(
  ...
  try {
    ...
    return jwt.sign(payload, getDocumentJwtSecret(), {
...
export function verifyDocumentToken(
  ...
  try {
    const payload = jwt.verify(token, getDocumentJwtSecret(), {
      algorithms: ["HS256"],
...
export function getDocumentUrl(
  ...
  const effectiveBaseUrl =
    baseUrl || getSecret("NEXT_PUBLIC_APP_URL", "http://localhost:3000");
