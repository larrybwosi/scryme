...
export async function main(
  ...
  const { organizationId, purchaseOrderId, orderNumber, requestedBy, totalAmount, currency } = data;

  console.log(`[PurchaseApproval] Processing PO ${orderNumber} for ${requestedBy}: ${currency} ${totalAmount}`);
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to submit PO approval callback: ${response.status} ${errorText}`);
  }
