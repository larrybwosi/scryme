...
export async function main(
  ...
  const { organizationId, adjustmentId, variantName, locationName, quantity, reason } = data;

  console.log(`[StockAdjustment] Monitoring ${adjustmentId}: ${variantName} at ${locationName} by ${quantity}`);
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to submit adjustment callback: ${response.status} ${errorText}`);
  }
