...
export async function main(
  ...
  console.log(`Processing ${data.eventType} for customer ${data.customerId} in org ${data.organizationId}`);
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch customer ${data.customerId}: ${response.statusText}`);
  }
  ...
  try {
    ...
    for (const member of membersToNotify) {
      ...
      const channels = [];
      if (tags.includes('notify:customer_created:email')) channels.push('EMAIL');
      if (tags.includes('notify:customer_created:discord')) channels.push('DISCORD');
      if (tags.includes('notify:customer_created:webhook')) channels.push('WEBHOOK');
    ...
    console.error(`Failed to send admin notifications: ${e instanceof Error ? e.message : String(e)}`);
  }
