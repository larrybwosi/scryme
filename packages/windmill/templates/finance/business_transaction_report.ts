...
export async function main(
  ...
  // 2. Aggregate metrics
  const totalRevenue = transactions.reduce((sum: number, tx: any) => sum + Number(tx.finalTotal), 0);
  const transactionCount = transactions.length;

  // Branch Performance
  const branchStats = new Map<string, { name: string; revenue: number; count: number }>();
  ...
  const paymentBreakdown = Array.from(paymentStats.entries()).map(([method, amount]) => ({
    method,
  ...
  transactions.forEach((tx: any) => {
    tx.items.forEach((item: any) => {
      ...
      const stats = productSales.get(key) || { name: item.productName, quantity: 0, revenue: 0 };
  ...
  try {
    ...
    console.error(`Failed to send business transaction report: ${error.message}`);
    throw error;
  }
