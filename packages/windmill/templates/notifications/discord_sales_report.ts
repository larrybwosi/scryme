...
export async function main(data: {
  ...
  const {
    organizationId,
    period,
    templateName = "DISCORD_SALES_REPORT",
  } = data;
  ...
  const totalRevenue = currentTransactions.reduce(
    (sum, tx) => sum + Number(tx.finalTotal),
    0,
  );
  ...
  const branchStats = new Map<
    string,
    { name: string; current: number; prev: number }
  >();
  ...
  const branchPerformance = Array.from(branchStats.values()).map((b) => ({
    ...
    growth:
      b.prev > 0
        ? (((b.current - b.prev) / b.prev) * 100).toFixed(2) + "%"
        : "N/A",
  }));
  ...
  currentTransactions.forEach((tx) => {
    tx.items.forEach((item) => {
      const stats = productSales.get(item.variantId) || {
