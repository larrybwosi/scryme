...
export async function main(
  ...
  console.log(`Triggering stale batch cleanup for organization: ${data.organizationId} (Grace period: ${gracePeriodHours}h)`);
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to trigger stale cleanup: ${response.status} ${errorText}`);
  }
