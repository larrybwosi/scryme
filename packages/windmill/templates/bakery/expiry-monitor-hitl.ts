...
async function checkExpiration(organizationId: string, env: any) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to run expiry logic: ${response.status} ${errorText}`,
    );
  }
...
async function submitDisposal(data: any, env: any) {
  console.log(
    `Submitting disposal decision for batch ${data.batchId} in org ${data.organizationId}`,
  );

  const response = await fetch(
    `${env.DEALIO_API_URL}/api/v3/webhooks/windmill/bakery/disposal`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(
      `Failed to submit disposal callback: ${response.status} ${errorText}`,
    );
  }
