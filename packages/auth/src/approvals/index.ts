...
export async function updateApprovalWorkflowInfo(
  workflowId: string,
  workflowUpdateData: any,
) {
  if (
    !workflowUpdateData.name &&
    !workflowUpdateData.description &&
    workflowUpdateData.isActive === undefined
  ) {
  ...
  try {
    ...
    return {
      success: true,
      message: `Workflow '${updatedWorkflow.name}' info updated.`,
      workflowId,
    };
  } catch (error) {
    ...
    return {
      success: false,
      message: "Failed to update workflow info.",
      error,
    };
  }
}
  ...
  try {
    if (workflowId) {
      ...
      if (!workflow || workflow.organizationId !== orgId) {
        return {
          success: false,
          message: "Workflow not found or does not belong to the organization.",
        };
      }
      if (!workflow.isActive) {
        return {
          success: false,
          message: "Cannot set an inactive workflow as active.",
        };
      }
    }
    ...
    const message = workflowId
      ...
      : "Expense approval workflow deactivated.";
